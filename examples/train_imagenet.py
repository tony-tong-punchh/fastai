...
def get_data(path, size, bs, workers):
    tfms = ([flip_lr(p=0.5), brightness(change=(0.4, 0.6)), contrast(scale=(0.7, 1.3))], [])
    train = ImageList.from_csv(path, "train.csv")
    valid = ImageList.from_csv(path, "valid.csv")
    lls = (
        ItemLists(path, train, valid)
        .label_from_df()
        .transform(tfms, size=size)
        .presize(size, scale=(0.25, 1.0))
    )
...
def main(gpu: Param("GPU to run on", str) = None):
    ...
    ph1 = TrainingPhase(tot_epochs * 0.10 * b_its).schedule_hp(
        "lr", (lr / 10, lr), anneal=annealing_cos
    )
    ph2 = TrainingPhase(tot_epochs * 0.90 * b_its).schedule_hp(
        "lr", (lr, lr / 1e5), anneal=annealing_cos
    )
