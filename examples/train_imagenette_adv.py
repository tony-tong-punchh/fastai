...
def get_data(size, woof, bs, workers=None, use_lighting=False):
    ...
    if size <= 128:
        path = path / ("imagewoof-160" if woof else "imagenette-160")
    elif size <= 192:
        path = path / ("imagewoof-320" if woof else "imagenette-320")
    else:
        path = path / ("imagewoof" if woof else "imagenette")
...
def get_learner(lr, size, woof, bs, opt_func, gpu, epochs):
    ...
    ph1 = (
        TrainingPhase(epochs * 0.5 * b_its)
...
def bn_and_final(m):
    ...
    idx = [i for i, o in enumerate(ll) if (i > 50 and isinstance(o, bn_types)) or o == last_lin]
...
def on_step(self, p, group, group_idx):
    st = self.state[p]
    alpha = (
        ((st["alpha_buffer"] + group["eps"]).sqrt())
        if "alpha_buffer" in st
        else mom.new_tensor(1.0)
    )
...
def main(
    ...
    stats = [
        ...
        ConstStatistic("eps", eps),
        ConstStatistic("clip", 0.001),
    ]
    if opt == "adam":
        opt_func = partial(optim.Adam, betas=(mom, alpha), eps=eps)
    elif opt == "rms":
        opt_func = partial(optim.RMSprop, alpha=alpha)
    elif opt == "genopt":
