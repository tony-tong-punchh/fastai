...
class DistributedTrainer(LearnerCallback):
    ...
    def on_train_begin(self, **kwargs):
        self.learn.model = DistributedDataParallel(
            self.model, device_ids=[self.cuda_id], output_device=self.cuda_id
        )
        shuffle = (
            self.data.train_dl.init_kwargs["shuffle"]
            if hasattr(self.data.train_dl, "init_kwargs")
            else True
        )
        shuffle_train = True if isinstance(self.data, TextClasDataBunch) else shuffle
        self.old_train_dl, self.data.train_dl, self.train_sampler = self._change_dl(
            self.data.train_dl, shuffle_train
        )
        if hasattr(self.data, "valid_dl") and self.data.valid_dl is not None:
            self.old_valid_dl, self.data.valid_dl, self.valid_sampler = self._change_dl(
                self.data.valid_dl, shuffle
            )
...
def _learner_parallel(learn: Learner):
    ...
    if not torch.cuda.is_available():
        warnings.warn(
            "CUDA is not available, check your drivers - training will continue on CPU",
            ResourceWarning,
        )
