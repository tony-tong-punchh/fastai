...
class DataBunch:
    ...
    def create(
        ...
        **dl_kwargs,
        ...
        dls = [
            DataLoader(d, b, shuffle=s, drop_last=s, num_workers=num_workers, **dl_kwargs)
    ...
    @property
    def fix_ds(self) -> Dataset:
        return self._grab_dataset(self.fix_dl)
    ...
    def loss_func(self) -> OptLossFunc:
        return (
            getattr(self.train_ds.y, "loss_func", F.nll_loss)
            if hasattr(self.train_ds, "y")
            else F.nll_loss
        )
    ...
    def empty_val(self) -> bool:
        if not hasattr(self, "valid_dl") or self.valid_dl is None:
            return True
        if hasattr(self.valid_ds, "items") and len(self.valid_ds.items) == 0:
            return True
    ...
    def is_empty(self) -> bool:
        return not (
            (self.train_dl and len(self.train_ds.items) != 0)
            or (self.valid_dl and len(self.valid_ds.items) != 0)
            or (self.test_dl and len(self.test_ds.items) != 0)
        )
    ...
    def sanity_check(self):
        ...
        if (
            ...
            or len(self.train_ds.items) == 0
            or not hasattr(self.train_dl, "batch_sampler")
        ):
            return
