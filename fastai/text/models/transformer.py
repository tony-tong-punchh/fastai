...
def feed_forward(
    ...
    return SequentialEx(
        *layers, nn.Linear(d_ff, d_model), nn.Dropout(ff_p), MergeLayer(), nn.LayerNorm(d_model)
    )
...
def _line_shift(x: Tensor, mask: bool = False):
    ...
    if mask:
        x_shift.mul_(
            torch.tril(x.new_ones(n, p), p - n)[None, None,]
        )
    return x_shift
...
class MultiHeadRelativeAttention(MultiHeadAttention):
    ...
    def __init__(
        ...
        super().__init__(
            n_heads, d_model, d_head, resid_p=resid_p, attn_p=attn_p, bias=bias, scale=scale
        )
...
class Transformer(Module):
    ...
    def __init__(
        ...
        self.pos_enc = (
            nn.Embedding(ctx_len, d_model) if learned_pos_enc else PositionalEncoding(d_model)
        )
    ...
    def forward(self, x):
        ...
        mask = (
            torch.triu(x.new_ones(x_len, x_len), diagonal=1).byte()[None, None]
            if self.mask
            else None
        )
...
class TransformerXL(Module):
    ...
    def __init__(
        ...
        act: Activation = Activation.ReLU,
        double_drop: bool = True,
        attn_cls: Callable = MultiHeadRelativeAttention,
        ...
        self.drop_emb = nn.Dropout(embed_p)
        self.u = nn.Parameter(
            torch.Tensor(n_heads, 1, d_head)
        )  # Remove 1 for einsum implementation of attention
        self.v = nn.Parameter(
            torch.Tensor(n_heads, 1, d_head)
        )  # Remove 1 for einsum implementation of attention
    ...
    def _update_mems(self, hids):
        if not getattr(self, "hidden", False):
            return None
    ...
    def forward(self, x):
        ...
        seq_len = m_len + x_len
        mask = (
            torch.triu(x.new_ones(x_len, seq_len), diagonal=1 + m_len).byte()[None, None]
            if self.mask
            else None
        )
        # [None,:,:None] for einsum implementation of attention
...
def init_transformer(m):
    ...
    if classname.find("Linear") != -1:
        ...
        if hasattr(m, "u"):
            nn.init.normal_(m.u, 0.0, 0.02)
        if hasattr(m, "v"):
            nn.init.normal_(m.v, 0.0, 0.02)


tfmer_lm_config = dict(
    ctx_len=512,
    n_layers=12,
    n_heads=12,
    d_model=768,
...
def tfmerXL_clas_split(model: nn.Module) -> List[nn.Module]:
    ...
    groups = [
        [encoder.encoder],
        list(encoder.layers[:n]) + [ParameterModule(encoder.u), ParameterModule(encoder.v)],
    ]
