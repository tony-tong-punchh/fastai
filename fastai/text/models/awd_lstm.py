...
class WeightDropout(Module):
    ...
    def _setweights(self):
        ...
        for i, layer in enumerate(self.layer_names):
            ...
            if self.idxs is not None:
                self.module._flat_weights[self.idxs[i]] = self.module._parameters[layer]
...
class AWD_LSTM(Module):
    ...
    def __init__(
        ...
        self.bs, self.qrnn, self.emb_sz, self.n_hid, self.n_layers = (
            1,
            qrnn,
            emb_sz,
            n_hid,
        ...
        if self.qrnn:
            ...
            for rnn in self.rnns:
                rnn.layers[0].linear = WeightDropout(
                    rnn.layers[0].linear, weight_p, layer_names=["weight"]
                )
        else:
            self.rnns = [
                nn.LSTM(
        ...
        if self.encoder.padding_idx is not None:
            self.encoder.weight.data[self.encoder.padding_idx] = 0.0
