...
if torch.cuda.is_available():
    ...
    bwd_forget_mult_cuda = load(
        name="bwd_forget_mult_cuda", sources=[fastai_path / f for f in files]
    )
...
class ForgetMultGPU(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_x, grad_f, grad_h = forget_mult_cuda.backward(
            x, f, output, grad_output, ctx.batch_first
        )
...
class BwdForgetMultGPU(Function):
    ...
    def backward(ctx, grad_output: Tensor):
        x, f, hidden_init, output = ctx.saved_tensors
        grad_x, grad_f, grad_h = bwd_forget_mult_cuda.backward(
            x, f, output, grad_output, ctx.batch_first
        )
        return (grad_x, grad_f, (None if hidden_init is None else grad_h), None)
...
def forget_mult_CPU(
    ...
    for i in idx_range:
        prev_h = (
            inputs[i] * forgets[i]
            if prev_h is None
            else inputs[i] * forgets[i] + (1 - forgets[i]) * prev_h
        )
        if backward:
...
class QRNNLayer(Module):
    ...
    def forward(self, inp, hid=None):
        ...
        if self.backward:
            forget_mult = dispatch_cuda(
                BwdForgetMultGPU, partial(forget_mult_CPU, backward=True), inp
            )
        else:
            forget_mult = dispatch_cuda(ForgetMultGPU, forget_mult_CPU, inp)
    ...
    def _get_source(self, inp):
        ...
        inp_shift = [
            torch.zeros_like(inp[:, :1] if self.batch_first else inp[:1])
...
class QRNN(Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                QRNNLayer(
                    ...
                    window=((2 if l == 0 else 1) if window is None else window),
                    **kwargs,
                )
        ...
        self.n_layers, self.batch_first, self.dropout, self.bidirectional = (
            n_layers,
            batch_first,
            dropout,
            bidirectional,
    ...
    def forward(self, inp, hid=None):
        ...
        for i, layer in enumerate(self.layers):
            ...
            new_hid.append(h)
            if self.bidirectional:
                inp_bwd, h_bwd = self.layers_bwd[i](
                    inp_bwd, None if hid is None else hid[2 * i + 1]
                )
