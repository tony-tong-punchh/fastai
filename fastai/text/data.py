...
class LanguageModelPreLoader(Callback):
    ...
    class CircularIndex:
        ...
        def __getitem__(self, i):
            return self.idx[
                i % len(self.idx) if self.forward else len(self.idx) - 1 - i % len(self.idx)
            ]
    ...
    def __init__(
        ...
        self.dataset, self.bs, self.bptt, self.shuffle, self.backwards, self.lengths = (
            dataset,
            bs,
            bptt,
            shuffle,
    ...
    def __len__(self):
        if self.ite_len is None:
            if self.lengths is None:
                self.lengths = np.array([len(item) for item in self.dataset.x.items])
    ...
    def allocate_buffers(self):
        ...
        if self.ite_len is None:
            len(self)
        self.idx = LanguageModelPreLoader.CircularIndex(
            len(self.dataset.x.items), not self.backwards
        )
    ...
    def on_epoch_begin(self, **kwargs):
        if self.idx is None or len(self.idx) != len(self.dataset.x.items):
...
def pad_collate(
    samples: BatchSamples, pad_idx: int = 1, pad_first: bool = True, backwards: bool = False
...
class TextDataBunch(DataBunch):
    ...
    def from_ids(
        ...
        train_lbls: Collection[Union[int, float]] = None,
        valid_lbls: Collection[Union[int, float]] = None,
        ...
        **kwargs,
        ...
        src = (
            ...
            else src.label_from_lists(train_lbls, valid_lbls, classes=classes, processor=[])
        )
        ...
        src.valid.x.processor = ifnone(
            processor, [TokenizeProcessor(), NumericalizeProcessor(vocab=vocab)]
        )
    ...
    def load(
        ...
        **kwargs,
        ...
        train_ids, train_lbls = (
            np.load(cache_path / f"train_ids.npy"),
            np.load(cache_path / f"train_lbl.npy"),
        )
        valid_ids, valid_lbls = (
            np.load(cache_path / f"valid_ids.npy"),
            np.load(cache_path / f"valid_lbl.npy"),
        )
        test_ids = (
            np.load(cache_path / f"test_ids.npy")
            if os.path.isfile(cache_path / f"test_ids.npy")
            else None
        )
        classes = (
            loadtxt_str(cache_path / "classes.txt")
            if os.path.isfile(cache_path / "classes.txt")
            else None
        )
        return cls.from_ids(
            path,
    ...
    def from_tokens(
        cls,
        path: PathOrStr,
        trn_tok: Collection[Collection[str]],
        trn_lbls: Collection[Union[int, float]],
        val_tok: Collection[Collection[str]],
        val_lbls: Collection[Union[int, float]],
        vocab: Vocab = None,
        tst_tok: Collection[Collection[str]] = None,
        classes: Collection[Any] = None,
        max_vocab: int = 60000,
        min_freq: int = 3,
        **kwargs,
        ...
        src = ItemLists(
            path,
            TextList(trn_tok, path=path, processor=processor),
            TextList(val_tok, path=path, processor=processor),
        )
        src = (
            src.label_for_lm()
            if cls == TextLMDataBunch
            else src.label_from_lists(trn_lbls, val_lbls, classes=classes)
        )
        if tst_tok is not None:
            src.add_test(TextList(tst_tok, path=path))
        return src.databunch(**kwargs)
    ...
    def from_csv(
        ...
        **kwargs,
        ...
        test_df = (
            None
            if test is None
            else pd.read_csv(Path(path) / test, header=header, delimiter=delimiter)
        )
        return cls.from_df(
            path,
            train_df,
    ...
    def from_folder(
        ...
        **kwargs,
        ...
        processor = [OpenFileProcessor()] + _get_processor(
            tokenizer=tokenizer,
            vocab=vocab,
            chunksize=chunksize,
            max_vocab=max_vocab,
        ...
        src = TextList.from_folder(path, processor=processor).split_by_folder(
            train=train, valid=valid
        )
        src = (
            src.label_for_lm() if cls == TextLMDataBunch else src.label_from_folder(classes=classes)
        )
        if test is not None:
...
class TextLMDataBunch(TextDataBunch):
    ...
    def create(
        ...
        **dl_kwargs,
        ...
        datasets = [
            LanguageModelPreLoader(
                ds, shuffle=(i == 0), bs=(bs if i == 0 else val_bs), bptt=bptt, backwards=backwards
            )
            for i, ds in enumerate(datasets)
        ]
        val_bs = bs
        dls = [
            DataLoader(d, b, shuffle=False, **dl_kwargs)
    ...
    def create(
        ...
        **dl_kwargs,
        ...
        train_sampler = SortishSampler(
            datasets[0].x, key=lambda t: len(datasets[0][t][0].data), bs=bs
        )
        train_dl = DataLoader(
            datasets[0], batch_size=bs, sampler=train_sampler, drop_last=True, **dl_kwargs
        )
...
class TokenizeProcessor(PreProcessor):
    ...
    def __init__(
        ...
        self.tokenizer, self.chunksize, self.mark_fields = (
            ifnone(tokenizer, Tokenizer()),
            chunksize,
            mark_fields,
        )
    ...
    def process_one(self, item):
        return self.tokenizer._process_all_1(
            _join_texts([item], self.mark_fields, self.include_bos, self.include_eos)
        )[0]
...
class TextList(ItemList):
    ...
    def from_folder(
        cls,
        path: PathOrStr = ".",
        extensions: Collection[str] = text_extensions,
        vocab: Vocab = None,
        processor: PreProcessor = None,
        **kwargs,
    ) -> "TextList":
        ...
        processor = ifnone(
            processor,
            [OpenFileProcessor(), TokenizeProcessor(), NumericalizeProcessor(vocab=vocab)],
        )
...
def _join_texts(
    ...
    text_col = (
        f"{bos_tok}{FLD} {1} " + df[0].astype(str)
        if mark_fields
        else f"{bos_tok}" + df[0].astype(str)
    )
...
class SPProcessor(PreProcessor):
    ...
    def __init__(
        ...
        self.sp_model, self.sp_vocab, self.n_cpus = (
            sp_model,
            sp_vocab,
            ifnone(n_cpus, defaults.cpus),
        )
    ...
    def process_one(self, item, join=True):
        if join:
            text = _join_texts([item], self.mark_fields, self.include_bos, self.include_eos)[0]
    ...
    def process(self, ds):
        ds.items = _join_texts(ds.items, self.mark_fields, self.include_bos, self.include_eos)
        ds.items = [
            apply_rules(t, pre_rules=self.pre_rules, post_rules=self.post_rules)
        ...
        if not getattr(self, "vocab", False):
            with open(self.sp_vocab, "r", encoding=self.enc) as f:
                self.vocab = Vocab([line.split("\t")[0] for line in f.readlines()])
        if self.n_cpus <= 1:
            ds.items = self._encode_batch(ds.items)
        else:
            with ProcessPoolExecutor(self.n_cpus) as e:
                ds.items = np.array(
                    sum(e.map(self._encode_batch, partition_by_cores(ds.items, self.n_cpus)), [])
                )
        ds.vocab = self.vocab
