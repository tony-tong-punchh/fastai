...
_model_meta = {
    ...
    Transformer: {
        "hid_name": "d_model",
        "url": URLs.OPENAI_TRANSFORMER,
        "config_lm": tfmer_lm_config,
        "split_lm": tfmer_lm_split,
        "config_clas": tfmer_clas_config,
        "split_clas": tfmer_clas_split,
    },
    TransformerXL: {
        "hid_name": "d_model",
        "config_lm": tfmerXL_lm_config,
        "split_lm": tfmerXL_lm_split,
        "config_clas": tfmerXL_clas_config,
        "split_clas": tfmerXL_clas_split,
    },
}
...
class RNNLearner(Learner):
    ...
    def __init__(
        ...
        **learn_kwargs,
        ...
        is_class = hasattr(data.train_ds, "y") and (
            isinstance(data.train_ds.y, CategoryList) or isinstance(data.train_ds.y, LMLabelList)
        )
        ...
        if clip:
            self.callback_fns.append(partial(GradientClipping, clip=clip))
        if split_func:
    ...
    def get_preds(
        ...
        if ordered:
            np.random.seed(42)
        preds = super().get_preds(
            ds_type=ds_type, activ=activ, with_loss=with_loss, n_batch=n_batch, pbar=pbar
        )
...
class LanguageLearner(RNNLearner):
    ...
    def predict(
        ...
        for _ in range(n_words):  # progress_bar(range(n_words), leave=False):
            ...
            if no_unk:
                res[self.data.vocab.stoi[UNK]] = 0.0
    ...
    def beam_search(
        ...
        with torch.no_grad():
            for k in progress_bar(range(n_words), leave=False):
                ...
                if no_unk:
                    out[:, self.data.vocab.stoi[UNK]] = -float("Inf")
        ...
        return (
            ...
            + sep
            + sep.join(
                decoder(self.data.vocab.textify([i.item() for i in nodes[node_idx][1:]], sep=None))
            )
        )
    ...
    **learn_kwargs,
    ...
    if pretrained or pretrained_fnames:
        if pretrained_fnames is not None:
            fnames = [
                learn.path / learn.model_dir / f"{fn}.{ext}"
                for fn, ext in zip(pretrained_fnames, ["pth", "pkl"])
            ]
        else:
            if url not in meta:
