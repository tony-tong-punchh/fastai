...
def loss_batch(
    ...
    if not loss_func:
        return to_detach(out), to_detach(yb[0])
...
def get_preds(
    ...
    res = [
        ...
        for o in zip(
            *validate(model, dl, cb_handler=cb_handler, pbar=pbar, average=False, n_batch=n_batch)
        )
    ]
...
def _loss_func_name2activ(name: str, axis: int = -1):
    ...
    if res == F.softmax:
        res = partial(F.softmax, dim=axis)
    return res


def _loss_func2activ(loss_func):
    if getattr(loss_func, "keywords", None):
        if not loss_func.keywords.get("log_input", True):
            return
    ...
    if cls_name in loss_func_name2activ:
        if cls_name == "poisson_nll_loss" and (not getattr(loss_func, "log_input", True)):
            return
...
class Learner:
    ...
    def __post_init__(self) -> None:
        ...
        if self.silent is None:
            self.silent = defaults.silent
        self.callback_fns = [
            partial(Recorder, add_time=self.add_time, silent=self.silent)
        ] + listify(self.callback_fns)
        if defaults.extra_callbacks is not None:
            self.callbacks += defaults.extra_callbacks
    ...
    def _test_writeable_path(self):
        ...
        try:
            ...
            raise Exception(
                f"{e}\nCan't write to '{path}', set `learn.model_dir` attribute in Learner to a full libpath path that is writable"
            ) from None
        os.remove(tmp_file)
    ...
    def fit(
        ...
        callbacks = [
            cb(self) for cb in self.callback_fns + listify(defaults.extra_callback_fns)
        ] + listify(callbacks)
    ...
    def export(self, file: PathLikeOrBinaryStream = "export.pkl", destroy=False):
        ...
        with ModelOnCPU(self.model) as m:
            state["model"] = m
            xtra = (
                dict(normalize=self.data.norm.keywords) if getattr(self.data, "norm", False) else {}
            )
    ...
    def destroy(self):
        ...
        methods = [
            k for k in dir(self) if not k.startswith("__") and inspect.isroutine(getattr(self, k))
        ]
        for m in methods:
    ...
    def purge(self, clear_opt: bool = True):
        ...
        if not clear_opt and "opt" in state:
            try:
                ...
                warn("Wasn't able to properly load the optimizer state again.")
        del state
        gc.collect()
        return self
    ...
    def pred_batch(
        ...
        with torch.no_grad():
            if not with_dropout:
                ...
                preds = loss_batch(
                    self.model.eval().apply(self.apply_dropout), xb, yb, cb_handler=cb_handler
                )
            res = activ(preds[0])
        if not reconstruct:
            return res
    ...
    def predict_with_mc_dropout(
        self, item: ItemBase, with_dropout: bool = True, n_times=10, **kwargs
    ):
...
class LearnerCallback(Callback):
    ...
    @learn.setter
    def learn(self, learn: Learner) -> None:
        self._learn = weakref.ref(learn)
...
class Recorder(LearnerCallback):
    ...
    def __init__(self, learn: Learner, add_time: bool = True, silent: bool = False):
        ...
        if not getattr(self.learn, "opt", False):
            self.learn.create_opt(defaults.lr, self.learn.wd)
    ...
    def on_train_begin(self, pbar: PBar, metrics_names: Collection[str], **kwargs: Any) -> None:
        ...
        self.pbar = pbar
        self.names = (
            ["epoch", "train_loss"] if self.no_val else ["epoch", "train_loss", "valid_loss"]
        )
        self.metrics_names = metrics_names
        if hasattr(self, "_added_met_names"):
            self.metrics_names += self._added_met_names
    ...
    def on_epoch_end(
        ...
        **kwargs: Any,
        ...
        if last_metrics is not None:
            ...
            last_metrics = [] if self.no_val else [None]
    ...
    def plot_lr(
        self, show_moms=False, skip_start: int = 0, skip_end: int = 0, return_fig: bool = None
    ) -> Optional[plt.Figure]:
...
def load_learner(
    ...
    **db_kwargs,
    ...
    state = (
        ...
        else torch.load(source)
    )
    model = state.pop("model")
    src = LabelLists.load_state(path, state.pop("data"))
    if test is not None:
        src.add_test(test, tfm_y=tfm_y)
