...
class TabularProcessor(PreProcessor):
    ...
    def process_one(self, item):
        ...
        if len(self.cat_names) != 0:
            codes = (
                np.stack([c.cat.codes.values for n, c in df[self.cat_names].items()], 1).astype(
                    np.int64
                )
                + 1
            )
        ...
        if len(self.cont_names) != 0:
            conts = np.stack(
                [c.astype("float32").values for n, c in df[self.cont_names].items()], 1
            )
        else:
            conts = [[]]
        classes = None
    ...
    def process(self, ds):
        ...
        if len(ds.cat_names) != 0:
            ds.codes = (
                np.stack(
                    [c.cat.codes.values for n, c in ds.inner_df[ds.cat_names].items()], 1
                ).astype(np.int64)
                + 1
            )
            self.classes = ds.classes = OrderedDict(
                {
                    n: np.concatenate([["#na#"], c.cat.categories.values])
        ...
        if len(ds.cont_names) != 0:
            ds.conts = np.stack(
                [c.astype("float32").values for n, c in ds.inner_df[ds.cont_names].items()], 1
            )
...
class TabularDataBunch(DataBunch):
    ...
    def from_df(
        ...
        procs = listify(procs)
        src = TabularList.from_df(
            df, path=path, cat_names=cat_names, cont_names=cont_names, procs=procs
        ).split_by_idx(valid_idx)
        src = (
            src.label_from_df(cols=dep_var)
            if classes is None
            else src.label_from_df(cols=dep_var, classes=classes)
        )
...
class TabularList(ItemList):
    ...
    def get(self, o):
        if not self.preprocessed:
            return self.inner_df.iloc[o] if hasattr(self, "inner_df") else self.items[o]
