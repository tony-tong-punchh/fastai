...
def pillow_version():
    ...
    try:
        return PIL.Image.__version__  # PIL >= 7
    except:
        return PIL.Image.PILLOW_VERSION  # PIL <  7
...
def pypi_module_version_is_available(module, version):
    ...
    try:
        ...
        result = subprocess.run(
            cmd.split(), shell=False, check=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def check_perf():
    ...
    if re.search(r"\.post\d+", pil_version):
        ...
        print(
            f"✘ Running Pillow {pil_version}; It's recommended you install Pillow-SIMD to speed up image resizing and other operations. See https://docs.fast.ai/performance.html#pillow-simd"
        )
    ...
    if torch.cuda.is_available():
        ...
        nvidia_ver = (
            pynvml.nvmlSystemGetDriverVersion().decode("utf-8")
            if platform.system() != "Darwin"
            else "Cannot be determined on OSX yet"
        )
