...
class OptimWrapper:
    ...
    def create(
        cls,
        opt_func: Union[type, Callable],
        lr: Union[float, Tuple, List],
    ...
    def step(self) -> None:
        ...
        if self.true_wd:
            for lr, wd, pg1, pg2 in zip(
                self._lr, self._wd, self.opt.param_groups[::2], self.opt.param_groups[1::2]
            ):
                for p in pg1["params"]:
    ...
    @property
    def n_params(self):
        return sum([len(pg["params"]) for pg in self.opt.param_groups])
    ...
    @property
    def mom(self) -> float:
        return self._mom[-1] if self._mom is not None else None
    ...
    def get_stat(self, name: str) -> float:
        if name in ["lr", "mom", "beta", "wd"]:
            return getattr(self, name)
        else:
            return self._stats[name][-1]
...
class Callback:
    ...
    def __repr__(self):
        ...
        list_repr = [self.__class__.__name__] + [
            f"{k}: {getattr(self, k)}" for k in attrs if k != "self" and k not in to_remove
        ]
        return "\n".join(list_repr)
...
class CallbackHandler:
    ...
    def __post_init__(self) -> None:
        ...
        self.metrics = [
            (met if isinstance(met, Callback) else AverageMetric(met)) for met in self.metrics
        ]
