...
def basic_critic(
    ...
    layers = [
        conv_layer(n_channels, n_features, 4, 2, 1, leaky=0.2, norm_type=None, **conv_kwargs)
    ]  # norm_type=None?
    cur_size, cur_ftrs = in_size // 2, n_features
    layers.append(
        nn.Sequential(
            *[
                conv_layer(cur_ftrs, cur_ftrs, 3, 1, leaky=0.2, **conv_kwargs)
...
def basic_generator(
    ...
    **conv_kwargs,
    ...
    while cur_size < in_size // 2:
        ...
        cur_size *= 2
    layers += [
        conv_layer(cur_ftrs, cur_ftrs, 3, 1, 1, transpose=True, **conv_kwargs)
        for _ in range(n_extra_layers)
    ]
...
class GANTrainer(LearnerCallback):
    ...
    def __init__(
        ...
        self.switch_eval, self.clip, self.beta, self.gen_first, self.show_img = (
            switch_eval,
            clip,
            beta,
            gen_first,
        ...
        self.generator, self.critic, self.switch_model = (
    ...
    def on_train_begin(self, **kwargs):
        ...
        if not getattr(self, "opt_critic", None):
            ...
            self.opt_critic.lr, self.opt_critic.wd = self.opt.lr, self.opt.wd
    ...
    def on_batch_begin(self, last_input, last_target, **kwargs):
        ...
        return (
            {"last_input": last_input, "last_target": last_target}
            if self.gen_mode
            else {"last_input": last_target, "last_target": last_input}
        )
    ...
    def on_epoch_end(self, pbar, epoch, last_metrics, **kwargs):
        ...
        return add_metrics(
            last_metrics,
            [getattr(self.smoothenerG, "smooth", None), getattr(self.smoothenerC, "smooth", None)],
        )
...
class AdaptiveGANSwitcher(LearnerCallback):
    ...
    def on_batch_end(self, last_loss, **kwargs):
        ...
        if self.gan_trainer.gen_mode:
            if self.gen_thresh is None:
                self.gan_trainer.switch()
            elif last_loss < self.gen_thresh:
                self.gan_trainer.switch()
        else:
            if self.critic_thresh is None:
                self.gan_trainer.switch()
            elif last_loss < self.critic_thresh:
                self.gan_trainer.switch()
...
class GANLearner(Learner):
    ...
    def from_learners(
        ...
        learn_crit: Learner,
        switcher: Callback = None,
        weights_gen: Tuple[float, float] = None,
        **learn_kwargs,
    ):
        "Create a GAN from `learn_gen` and `learn_crit`."
        losses = gan_loss_from_func(
            learn_gen.loss_func, learn_crit.loss_func, weights_gen=weights_gen
        )
        return cls(
            learn_gen.data,
            learn_gen.model,
