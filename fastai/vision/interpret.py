...
class SegmentationInterpretation(Interpretation):
    ...
    def show_xyz(self, i, classes: list = None, sz=10):
        ...
        self._interp_show(
            ImageSegment(self.pred_class[i][None, :]), classes, sz=sz, title_suffix="pred"
        )
    ...
    def _generate_confusion(self):
        ...
        self.single_img_cm = to_np(
            torch.stack(single_img_confusion).permute(1, 0).view(-1, self.data.c, self.data.c)
        )
    ...
    def _plot_intersect_cm(self, cm, title="Intersection with Predict given True"):
        ...
        df = pd.DataFrame(
            [self.data.classes, cm.diagonal()], index=["label", "score"]
