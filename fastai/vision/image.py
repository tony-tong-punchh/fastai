...
def _draw_rect(ax: plt.Axes, b: Collection[int], color: str = "white", text=None, text_size=14):
    ...
    if text is not None:
        patch = ax.text(
            *b[:2], text, verticalalignment="top", color=color, fontsize=text_size, weight="bold"
        )
...
class Image(ItemBase):
    ...
    def apply_tfms(
        ...
        xtra: Optional[Dict[Callable, dict]] = None,
        size: Optional[Union[int, TensorImageSize]] = None,
        resize_method: ResizeMethod = None,
        mult: int = None,
        padding_mode: str = "reflection",
        mode: str = "bilinear",
        remove_out: bool = True,
    ) -> TensorImage:
        "Apply all `tfms` to the `Image`, if `do_resolve` picks value for random args."
        if not (tfms or xtra or size):
            return self
        tfms = listify(tfms)
        xtra = ifnone(xtra, {})
        default_rsz = (
            ResizeMethod.SQUISH if (size is not None and is_listy(size)) else ResizeMethod.CROP
        )
        resize_method = ifnone(resize_method, default_rsz)
        if resize_method <= 2 and size is not None:
            tfms = self._maybe_add_crop_pad(tfms)
        ...
        if size is not None:
            ...
            if resize_method in (ResizeMethod.CROP, ResizeMethod.PAD):
                target = _get_resize_target(
                    x, crop_target, do_crop=(resize_method == ResizeMethod.CROP)
                )
                x.resize(target)
            elif resize_method == ResizeMethod.SQUISH:
                x.resize((x.shape[0],) + crop_target)
        else:
            size = x.size
    ...
    def resize(self, size: Union[int, TensorImageSize]) -> "Image":
        ...
        if isinstance(size, int):
            size = (self.shape[0], size, size)
        if tuple(size) == tuple(self.shape):
            return self
    ...
    def logit_px(self) -> LogitTensorImage:
        "Get logit(image.px)."
        if self._logit_px is None:
            self._logit_px = logit_(self.px)
        return self._logit_px

    @logit_px.setter
    def logit_px(self, v: LogitTensorImage) -> None:
        self._logit_px = v
...
class ImagePoints(Image):
    ...
    def __init__(self, flow: FlowField, scale: bool = True, y_first: bool = True):
        ...
        if y_first:
            flow.flow = flow.flow.flip(1).float()
        self._flow = flow
        self._affine_mat = None
        self.flow_func = []
        self.sample_kwargs = {}
    ...
    def clone(self):
        ...
        return self.__class__(
            FlowField(self.size, self.flow.flow.clone()), scale=False, y_first=False
        )

    @property
    def shape(self) -> Tuple[int, int, int]:
        return (1, *self._flow.size)
    ...
    @property
    def device(self) -> torch.device:
        return self._flow.flow.device

    def __repr__(self):
        return f"{self.__class__.__name__} {tuple(self.size)}"
    ...
    def flow(self) -> FlowField:
        ...
        if len(self.flow_func) != 0:
            for f in self.flow_func[::-1]:
                self._flow = f(self._flow)
            self.transformed = True
            self.flow_func = []
        return self._flow
    ...
    def lighting(self, func: LightingFunc, *args: Any, **kwargs: Any) -> "ImagePoints":
        return self
...
class ImageBBox(ImagePoints):
    ...
    def clone(self) -> "ImageBBox":
        ...
        return self.__class__(
            flow, scale=False, y_first=False, labels=self.labels, pad_idx=self.pad_idx
        )
    ...
    def create(
        ...
        h: int,
        w: int,
        bboxes: Collection[Collection[int]],
        labels: Collection = None,
        classes: dict = None,
        pad_idx: int = 0,
        scale: bool = True,
        ...
        if isinstance(bboxes, np.ndarray) and bboxes.dtype == np.object:
            bboxes = np.array([bb for bb in bboxes])
...
class Transform:
    ...
    def __init__(self, func: Callable, order: Optional[int] = None):
        ...
        setattr(Image, func.__name__, lambda x, *args, **kwargs: self.calc(x, *args, **kwargs))
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        if args:
            ...
            return RandTransform(self, kwargs=kwargs, is_random=is_random, use_on_y=use_on_y, p=p)
...
class RandTransform:
    ...
    def resolve(self) -> None:
        ...
        for k, v in self.tfm.params.items():
            if k not in self.resolved and k != "return":
                self.resolved[k] = v()
...
def _affine_grid(size: TensorImageSize) -> FlowField:
    ...
    linear_points = torch.linspace(-1, 1, W) if W > 1 else tensor([-1.0])
    grid[:, :, :, 0] = torch.ger(torch.ones(H), linear_points).expand_as(grid[:, :, :, 0])
    linear_points = torch.linspace(-1, 1, H) if H > 1 else tensor([-1.0])
...
def _get_resize_target(img, crop_target, do_crop=False) -> TensorImageSize:
    ...
    return (
        ...
        int(round(r / ratio)),
        int(round(c / ratio)),
    )  # Sometimes those are numpy numbers and round doesn't return an int.
...
def plot_multi(
    func: Callable[[int, int, plt.Axes], None], r: int = 1, c: int = 1, figsize: Tuple = (12, 6)
):
