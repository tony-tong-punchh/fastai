...
def _cl_int_from_learner(
    ...
    preds = (
        learn.TTA(ds_type=ds_type, with_loss=True)
        if tta
        else learn.get_preds(ds_type=ds_type, activ=activ, with_loss=True)
    )
...
def _cl_int_gradcam(
    ...
    if (acts.shape[-1] * acts.shape[-2]) >= heatmap_thresh:
        ...
        if image:
            ...
            ax.imshow(
                mult, alpha=0.4, extent=(0, *sz[::-1], 0), interpolation="bilinear", cmap="magma"
            )
        return mult
...
def _cl_int_plot_top_losses(
    ...
    assert not heatmap or _test_cnn(
        self.learn.model
    ), "`heatmap=True` requires a model like `cnn_learner` produces."
    if heatmap is None:
        heatmap = _test_cnn(self.learn.model)
    ...
    for i, idx in enumerate(tl_idx):
        ...
        if heatmap:
            ...
            if mult is not None:
                sz = list(im.shape[-2:])
                axes.flat[i].imshow(
                    mult, alpha=alpha, extent=(0, *sz[::-1], 0), interpolation="bilinear", cmap=cmap
                )
