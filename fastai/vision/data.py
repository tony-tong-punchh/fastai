...
def denormalize(
    x: TensorImage, mean: FloatTensor, std: FloatTensor, do_x: bool = True
) -> TensorImage:
...
class ImageDataBunch(DataBunch):
    ...
    def from_folder(
        cls,
        path: PathOrStr,
        train: PathOrStr = "train",
        valid: PathOrStr = "valid",
        test: Optional[PathOrStr] = None,
        valid_pct=None,
        seed: int = None,
        ...
        **kwargs: Any,
        ...
        if valid_pct is None:
            ...
            src = il.split_by_rand_pct(valid_pct, seed)
    ...
    def from_df(
        ...
        **kwargs: Any,
        ...
        src = (
            ImageList.from_df(df, path=path, folder=folder, suffix=suffix, cols=fn_col)
    ...
    def from_csv(
        ...
        folder: PathOrStr = None,
        label_delim: str = None,
        csv_labels: PathOrStr = "labels.csv",
        valid_pct: float = 0.2,
        seed: int = None,
        fn_col: int = 0,
        label_col: int = 1,
    ...
    @classmethod
    def from_name_re(
        cls, path: PathOrStr, fnames: FilePathList, pat: str, valid_pct: float = 0.2, **kwargs
    ):
    ...
    @staticmethod
    def single_from_classes(
        path: Union[Path, str], classes: Collection[str], ds_tfms: TfmList = None, **kwargs
    ):
        ...
        return (
            sd.label_const(0, label_cls=CategoryList, classes=classes)
            .transform(ds_tfms, **kwargs)
            .databunch()
        )

    def batch_stats(
        self, funcs: Collection[Callable] = None, ds_type: DatasetType = DatasetType.Train
    ) -> Tensor:
...
def download_images(
    urls: Union[Path, str], dest: PathOrStr, max_pics: int = 1000, max_workers: int = 8, timeout=4
):
    ...
    **kwargs,
    ...
    try:
        ...
        if (
            ...
            if resume and os.path.isfile(dest_fname):
                return
...
class ImageList(ItemList):
    ...
    @classmethod
    def from_csv(
        cls, path: PathOrStr, csv_name: str, header: str = "infer", delimiter: str = None, **kwargs
    ) -> "ItemList":
    ...
    def show_xys(
        self, xs, ys, imgsize: int = 4, figsize: Optional[Tuple[int, int]] = None, **kwargs
    ):
    ...
    def show_xyzs(
        self, xs, ys, zs, imgsize: int = 4, figsize: Optional[Tuple[int, int]] = None, **kwargs
    ):
        ...
        if self._square_show_res:
            ...
            rows = int(np.ceil(math.sqrt(len(xs))))
            axs = subplots(
                rows, rows, imgsize=imgsize, figsize=figsize, title=title, weight="bold", size=12
            )
            ...
            axs = subplots(
                len(xs), 2, imgsize=imgsize, figsize=figsize, title=title, weight="bold", size=14
            )
...
class ObjectCategoryProcessor(MultiCategoryProcessor):
    ...
    def process_one(self, item):
        return [item[0], [self.c2i.get(o, None) for o in item[1]]]
...
class ObjectCategoryList(MultiCategoryList):
    ...
    def get(self, i):
        return ImageBBox.create(
            *_get_size(self.x, i), *self.items[i], classes=self.classes, pad_idx=self.pad_idx
        )
...
class SegmentationLabelList(ImageList):
    ...
    def open(self, fn):
        return open_mask(fn, after_open=self.after_open)
...
class ImageImageList(ImageList):
    ...
    def show_xyzs(
        ...
        axs = subplots(
            len(xs), 3, imgsize=imgsize, figsize=figsize, title=title, weight="bold", size=14
        )
        for i, (x, y, z) in enumerate(zip(xs, ys, zs)):
            x.show(ax=axs[i, 0], **kwargs)
...
def _presize(
    self,
    size: int,
    val_xtra_size: int = 32,
    scale: Tuple[float] = (0.08, 1.0),
    ratio: Tuple[float] = (0.75, 4.0 / 3.0),
    interpolation: int = 2,
):
