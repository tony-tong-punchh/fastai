...
def _crop_image_points(x, size, row_pct=0.5, col_pct=0.5):
    ...
    x.flow.flow.add_(
        -1 + torch.Tensor([w / cols - 2 * col / cols, h / rows - 2 * row / rows])[None]
    )
    x.size = (rows, cols)
    return x
...
if _solve_func is None:
    _solve_func = torch.gesv
...
def _tilt(c, direction: uniform_int, magnitude: uniform = 0, invert=False):
    ...
    if direction == 0:
        ...
        targ_pts = [[-1 - magnitude, -1], [-1, 1], [1 + magnitude, -1], [1, 1]]
...
def _skew(c, direction: uniform_int, magnitude: uniform = 0, invert=False):
    ...
    if direction == 0:
        ...
        targ_pts = [[-1, -1], [-1, 1 + magnitude], [1, -1], [1, 1]]
    elif direction == 4:
        targ_pts = [[-1, -1], [-1, 1], [1 + magnitude, -1], [1, 1]]
    elif direction == 5:
        targ_pts = [[-1, -1], [-1, 1], [1, -1 - magnitude], [1, 1]]
    elif direction == 6:
        targ_pts = [[-1, -1], [-1, 1], [1, -1], [1 + magnitude, 1]]
    elif direction == 7:
        targ_pts = [[-1, -1], [-1, 1], [1, -1], [1, 1 + magnitude]]
    coeffs = _find_coeffs(targ_pts, _orig_pts) if invert else _find_coeffs(_orig_pts, targ_pts)
    return _apply_perspective(c, coeffs)
...
def get_transforms(
    ...
    if max_zoom > 1:
        res.append(rand_zoom(scale=(1.0, max_zoom), p=p_affine))
    if max_lighting:
        res.append(
...
def _compute_zs_mat(
    sz: TensorImageSize, scale: float, squish: float, invert: bool, row_pct: float, col_pct: float
) -> AffineMatrix:
    ...
    if orig_ratio > 1:
        return _get_zoom_mat(1 / orig_ratio ** 2, 1, 0, 0.0)
    else:
        return _get_zoom_mat(1, orig_ratio ** 2, 0, 0.0)
...
def rand_resize_crop(size: int, max_scale: float = 2.0, ratios: Tuple[float, float] = (0.75, 1.33)):
