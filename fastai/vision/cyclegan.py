...
def convT_norm_relu(
    ...
    return [
        nn.ConvTranspose2d(
            ch_in, ch_out, kernel_size=ks, stride=stride, padding=1, output_padding=1, bias=bias
        ),
        norm_layer(ch_out),
        nn.ReLU(True),
    ]
...
class CycleGanLoss(Module):
    ...
    def forward(self, output, target):
        ...
        idt_loss = self.l_idt * (
            self.l_B * F.l1_loss(idt_A, self.real_B) + self.l_A * F.l1_loss(idt_B, self.real_A)
        )
