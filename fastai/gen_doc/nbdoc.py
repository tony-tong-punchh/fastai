...
def format_ft_def(func, full_name: str = None) -> str:
    ...
    fmt_params = [
        format_param(param) for name, param in sig.parameters.items() if name not in ("self", "cls")
    ]
...
def show_doc(
    ...
    if inspect.isclass(elt):
        if is_enum(elt.__class__):
            name, args = get_enum_doc(elt, full_name)
        else:
            name, args = get_cls_doc(elt, full_name)
    elif isinstance(elt, Callable):
        name, args = format_ft_def(elt, full_name)
    else:
        raise Exception(f"doc definition not supported for {full_name}")
...
def doc(elt):
    ...
    use_relative_links = True
    if IS_IN_COLAB:
        get_ipython().run_cell_magic("html", "", output)
    else:
        try:
...
def format_docstring(
    ...
    args = (
        inspect.getfullargspec(elt).args if not is_enum(elt.__class__) else elt.__members__.keys()
    )
    if resolved_comments:
        parsed += "\n"
    for a in resolved_comments:
        parsed += f"\n- *{a}*: {resolved_comments[a]}"
        if a not in args and not ignore_warn:
            warn(f"Doc arg mismatch: {a}")
...
def get_inner_fts(elt) -> List[str]:
    ...
    for ft_name in elt.__dict__.keys():
        ...
        if inspect.ismethod(ft):
            fts.append(f"{elt.__name__}.{ft_name}")
        if inspect.isclass(ft):
            fts += [f"{elt.__name__}.{n}" for n in get_inner_fts(ft)]
    return fts
