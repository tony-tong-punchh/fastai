...
class TestRegistry:
    ...
    def this_tests(*funcs):
        ...
        entry = {
            "file": relative_test_path(file_name),
            "test": test_name,
            "line": parent_func_lineno,
        }
        for func in funcs:
            if func == "na":
    ...
    def registry_save():
        if TestRegistry.registry:
            ...
            with open(path, "w") as f:
                json.dump(
                    obj=TestRegistry.registry,
                    fp=f,
                    indent=4,
                    sort_keys=True,
                    default=_json_set_default,
                )
...
def get_func_fq_name(func):
    ...
    if hasattr(func, "__qualname__"):
        ...
        return get_func_fq_name(func.__wrapped__)
    elif hasattr(func, "__class__"):
        name = func.__class__.__name__
    else:
        raise Exception(f"'{func}' is not a func or class")
