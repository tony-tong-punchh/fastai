...
def _symbol_skeleton(name):
    return [get_doc_cell(name), get_md_cell(f"`{name}`")]
...
def create_module_page(mod, dest_path, force=False):
    ...
    if gvar_map:
        cells.append(get_md_cell("### Global Variable Definitions:"))
    for name in get_exports(mod):
        if name in gvar_map:
            cells.append(get_md_cell(gvar_map[name]))
...
def generate_missing_metadata(dest_file):
    ...
    fmt_params = ""
    for k, v in jmd.items():
        fmt_params += f",\n    {k}={stringify(v)}"
    metadata_cell = get_code_cell(
        f"update_nb_metadata('{Path(fn).name}'{fmt_params})", hidden=False
    )
...
def get_imported_modules(cells, nb_module_name=""):
    ...
    parent_modules = [
        ".".join(parts[: (x + 1)]) for x in range_of(parts)
...
def update_module_page(mod, dest_path="."):
    ...
    for name in get_exports(mod):
        ...
        if name in type_dict:
            cells[type_dict[name]] = get_md_cell(code)
        else:
            cells.append(get_md_cell(code))
    ...
    for ft_name in new_fts:
        new_cells.extend([get_doc_cell(ft_name), get_empty_cell()])
    if len(new_cells) > 1:
        nb["cells"] = existing + undoc_cells + new_cells
...
def update_notebooks(
    ...
    if source_path.is_file():
        ...
        if document_new_fns:
            ...
            if not mod:
                print("Could not find module for path:", source_path)
            elif mod.__file__.endswith("__init__.py"):
                pass
            else:
        ...
        if not mod:
            return print("Could not find module for:", source_path)
        ...
        update_notebooks(
            ...
            document_new_fns=document_new_fns,
            update_nb_links=update_nb_links,
            html_path=html_path,
        )
        ...
        print("Could not resolve source file:", source_path)
