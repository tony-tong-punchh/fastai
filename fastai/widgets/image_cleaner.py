...
class DatasetFormatter:
    ...
    @staticmethod
    def padded_ds(
        ll_input, size=(250, 300), resize_method=ResizeMethod.CROP, padding_mode="zeros", **kwargs
    ):
        "For a LabelList `ll_input`, resize each image to `size` using `resize_method` and `padding_mode`."
        return ll_input.transform(
            tfms=crop_pad(), size=size, resize_method=resize_method, padding_mode=padding_mode
        )
    ...
    @staticmethod
    def get_actns(
        learn, hook: Hook, dl: DataLoader, pool=AdaptiveConcatPool2d, pool_dim: int = 4, **kwargs
    ):
    ...
    @classmethod
    def from_most_unsure(
        cls, learn: Learner, num=50
    ) -> Tuple[DataLoader, List[int], Sequence[str], List[str]]:
        """
...
class BasicImageWidget(ABC):
    def __init__(
        ...
        batch_size=5,
        drop_batch_on_nonfile=False,
        classes: Optional[Sequence[str]] = None,
        labels: Optional[Sequence[str]] = None,
        before_next_batch: Optional[Callable[[Tuple[Mapping, ...]], Any]] = None,
    ):
        super().__init__()
        self._dataset, self.batch_size, self._labels, self.before_next_batch = (
            dataset,
            batch_size,
            labels,
            before_next_batch,
    ...
    @staticmethod
    def make_img_widget(
        img: bytes, layout=Layout(height="250px", width="300px"), format="jpg"
    ) -> widgets.Image:
    ...
    @staticmethod
    def make_vertical_box(
        children: Collection[widgets.Widget],
    ...
    def create_image_list(
        self, fns_idxs: Collection[int], drop_batch_on_nonfile=False
    ) -> Iterator[ImgData]:
        ...
        idxs = (
            (i for i in fns_idxs if Path(items[i]).is_file())
            if not drop_batch_on_nonfile
            else chain.from_iterable(
                c
                for c in chunks(fns_idxs, self.batch_size)
                if all(Path(items[i]).is_file() for i in c)
            )
        )
        for i in idxs:
            yield ImgData(
                self._dataset.x[i]._repr_jpeg_(), self._get_label(i), self.make_payload(i)
            )
    ...
    def _get_label(self, idx):
        ...
        return (
            ...
            else self._classes[self._dataset.y[idx].data]
        )
...
class ImageCleaner(BasicImageWidget):
    ...
    def __init__(
        ...
        super().__init__(
            dataset,
            fns_idxs,
            batch_size=(2 if duplicates else batch_size),
            drop_batch_on_nonfile=duplicates,
            before_next_batch=self.before_next_batch,
        )
    ...
    def make_payload(self, idx: int):
        return {"file_path": self._dataset.x.items[idx]}
    ...
    def get_widgets(self, batch: Tuple[ImgData]) -> List[widgets.Widget]:
        ...
        for i, img in enumerate(batch):
            ...
            if not self._duplicates:
                dropdown = self.make_dropdown_widget(
                    options=self._classes, value=img.label, handler=self.relabel, img_idx=i
                )
            delete_btn = self.make_button_widget("Delete", handler=self.on_delete, img_idx=i)
            widgets.append(
                self.make_vertical_box(
                    (img_widget, delete_btn)
                    if self._duplicates
                    else (img_widget, dropdown, delete_btn)
                )
            )
        return widgets
    ...
    def batch_contains_deleted(self, batch: Tuple[ImgData]):
        ...
        return self._duplicates and any(
            img.payload["file_path"] in self._deleted_fns for img in batch
        )
