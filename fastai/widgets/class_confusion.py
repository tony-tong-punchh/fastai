...
class ClassConfusion:
    ...
    def _create_tabs(self):
        ...
        if self._is_tab:
            ...
            self.tbnames = (
                list(self.df_list[0].columns)[:-1] if self.varlist is None else self.varlist
            )
        else:
            ...
            self.tbnames = []
            self._boxes = int(
                input("Please enter a value for `k`, or the top images you will see: ")
            )
            for x in iter(vals):
    ...
    def _plot_tab(self, tab: str):
        ...
        for j, x in enumerate(self.df_list):
            ...
            if tab in self.cat_names:
                ...
                if self._boxes is not None:
                    if vals.nunique() < 10:
                        fig = vals.plot(kind="bar", title=title, ax=ax[j], rot=0, width=0.75)
                    elif vals.nunique() > self.cut_off:
                        print(f"Number of values is above {self.cut_off}")
                    else:
                        fig = vals.plot(kind="barh", title=title, ax=ax[j], width=0.75)
                else:
                    fig = vals.plot(kind="barh", title=title, ax=ax[row, col], width=0.75)
            else:
                vals = x[tab]
                if self._boxes is not None:
    ...
    def _plot_imgs(self, tab: str, i: int, **kwargs):
        ...
        for j, idx in enumerate(self.tl_idx):
            ...
            if (
                str(cl) == tab.split(" ")[0]
                and str(classes_gnd[self.interp.pred_class[idx]]) == tab.split(" ")[2]
            ):
