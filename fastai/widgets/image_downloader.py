...
class ImageDownloader:
    ...
    def _init_ui(self) -> VBox:
        ...
        self._count_input = BoundedIntText(
            placeholder="How many pics?",
            value=10,
            min=1,
            max=5000,
            step=1,
        ...
        self._size_input = Dropdown(
            options=_img_sizes.keys(), value=">400*300", layout=Layout(width="120px")
        )
        self._download_button = Button(
            description="Search & Download", icon="download", layout=Layout(width="200px")
        )
        self._download_button.on_click(self.on_download_button_click)
        self._output = Output()
        self._controls_pane = HBox(
            [self._search_input, self._count_input, self._size_input, self._download_button],
    ...
    def on_download_button_click(self, btn) -> None:
        ...
        if not self.validate_search_input():
            return
        self.clear_imgs()
...
def download_google_images(
    ...
    downloaded_images = _download_images(
        label_path, img_tuples, max_workers=max_workers, timeout=timeout
    )
    if len(downloaded_images) == 0:
        raise RuntimeError(f"Couldn't download any images.")
...
def _download_images(
    label_path: PathOrStr, img_tuples: list, max_workers: int = defaults.cpus, timeout: int = 4
) -> FilePathList:
    """
    ...
    parallel(
        partial(_download_single_image, label_path, timeout=timeout),
        img_tuples,
