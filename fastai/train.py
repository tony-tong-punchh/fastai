...
def fit_fc(
    ...
    callbacks = listify(callbacks)
    callbacks.append(
        FlatCosAnnealScheduler(learn, lr, moms=moms, start_pct=start_pct, tot_epochs=tot_epochs)
    )
...
def to_fp16(
    ...
    learn.mp_cb = MixedPrecision(
        learn,
        loss_scale=loss_scale,
        max_noskip=max_noskip,
        dynamic=dynamic,
        clip=clip,
...
class ShowGraph(LearnerCallback):
    ...
    def on_epoch_end(self, n_epochs: int, last_metrics: MetricsList, **kwargs) -> bool:
        ...
        if last_metrics is not None and last_metrics[0] is not None:
            ...
            rec.pbar.update_graph(
                [(iters, rec.losses), (val_iter, rec.val_losses)], x_bounds, y_bounds
            )
        return {}
...
class AccumulateScheduler(LearnerCallback):
    ...
    def on_epoch_begin(self, **kwargs):
        ...
        self.acc_samples, self.acc_batches = 0.0, 0.0
...
class Interpretation:
    ...
    def __init__(
        ...
        self.ds = (
            ...
            else self.data.test_ds
            if ds_type == DatasetType.Test
            else self.data.valid_ds
            if ds_type == DatasetType.Valid
            else self.data.single_ds
            if ds_type == DatasetType.Single
            else self.data.fix_ds
        )

    @classmethod
    def from_learner(
        cls, learn: Learner, ds_type: DatasetType = DatasetType.Valid, activ: nn.Module = None
    ):
...
class ClassificationInterpretation(Interpretation):
    ...
    def confusion_matrix(self, slice_size: int = 1):
        ...
        if slice_size is None:
            ...
            for i in range(0, self.y_true.shape[0], slice_size):
                cm_slice = (
                    (self.pred_class[i : i + slice_size] == x[:, None])
                    & (self.y_true[i : i + slice_size] == x[:, None, None])
                ).sum(2)
    ...
    def plot_confusion_matrix(
        ...
        **kwargs,
        ...
        ax = fig.gca()
        ax.set_ylim(len(self.data.y.classes) - 0.5, -0.5)
    ...
    def most_confused(
        self, min_val: int = 1, slice_size: int = 1
    ) -> Collection[Tuple[str, str, int]]:
        "Sorted descending list of largest non-diagonal entries of confusion matrix, presented as actual, predicted, number of occurrences."
        cm = self.confusion_matrix(slice_size=slice_size)
        np.fill_diagonal(cm, 0)
        res = [
            (self.data.classes[i], self.data.classes[j], cm[i, j])
...
class MultiLabelClassificationInterpretation(Interpretation):
    ...
    def __init__(
        ...
        super(MultiLabelClassificationInterpretation, self).__init__(
