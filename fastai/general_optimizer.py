...
class AvgSquare(AvgStatistic):
    ...
    def _get_val1(self, val):
        return torch.norm(val).pow(2) / val.numel()
...
class GeneralOptimizer(Optimizer):
    ...
    def _init_stats(self, stats, data=None):
        return {
            stat.buf: stat.init if data is None else torch.zeros_like(data) + stat.init
            for stat in stats
    ...
    def init_stats(self):
        ...
        for i, pg in enumerate(self.param_groups):
            ...
            for p in pg["params"]:
                ...
                self.state[p].update(
                    self._init_stats(self.channel_stats, p.data.view(p.data.size(0), -1).mean(1))
                )
    ...
    def _set_bufs(self, p, stats, pg, val=None):
        ...
        for stat in stats:
            if stat.buf is not None:
                d[stat.buf] = stat.update(
                    d[stat.buf], pg[stat.name], val=val, step=d.get("step", None)
                )
        ...
        for i, pg in enumerate(self.param_groups):
            ...
            for p in pg["params"]:
                if p.grad is not None:
                    ...
                    self._set_bufs(
                        p,
                        self.layer_stats + self.channel_stats + self.weight_stats,
                        pg,
                        p.grad.data,
                    )
