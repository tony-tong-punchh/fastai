...
def _decode(df):
    return np.array(
        [[df.columns[i] for i, t in enumerate(x) if t == 1] for x in df.values], dtype=np.object
    )
...
def _maybe_squeeze(arr):
    return (
        arr
        if is1d(arr)
        else (np.array(arr).ravel() if np.array(arr).shape == () else np.squeeze(arr))
    )
...
class ItemList:
    ...
    def __init__(
        ...
        self.label_cls, self.inner_df, self.processor = (
            ifnone(label_cls, self._label_cls),
            inner_df,
            processor,
        )
    ...
    def reconstruct(self, t: Tensor, x: Tensor = None):
        ...
        if not hasattr(self[0], "reconstruct"):
            return t
        return (
            self[0].reconstruct(t, x)
            if has_arg(self[0].reconstruct, "x")
            else self[0].reconstruct(t)
        )
    ...
    def from_folder(
        ...
        **kwargs,
        ...
        return cls(
            get_files(
                path, extensions, recurse=recurse, exclude=exclude, include=include, presort=presort
            ),
            path=path,
            processor=processor,
            **kwargs,
    ...
    def from_df(
        ...
        **kwargs,
        ...
        assert (
            not inputs.isna().any().any()
        ), f"You have NaN values in column(s) {cols} of your dataframe, please fix it."
        items = (
            _maybe_squeeze(inputs.values)
            if len(df) > 1
            else (
                inputs.values[0]
                if not isinstance(cols, Collection) or len(cols) == 1
                else inputs.values
            )
        )
    ...
    def _get_by_folder(self, name):
        return [
            ...
            if (
                self.items[i].parts[self.num_parts]
                if isinstance(self.items[i], Path)
                else self.items[i].split(os.path.sep)[0]
            )
            == name
        ]
    ...
    def split_subsets(self, train_size: float, valid_size: float, seed=None) -> "ItemLists":
        ...
        assert train_size + valid_size <= 1.0
        if seed is not None:
            np.random.seed(seed)
        n = len(self.items)
    ...
    def split_by_files(self, valid_names: "ItemList") -> "ItemLists":
        ...
        if isinstance(self.items[0], Path):
...
It's either because your data source is empty or because your labelling function raised an error."""
    ...
    def label_from_df(self, cols: IntsOrStrs = 1, label_cls: Callable = None, **kwargs):
        ...
        assert (
            labels.isna().sum().sum() == 0
        ), f"You have NaN values in column(s) {cols} of your dataframe, please fix it."
        if (
            is_listy(cols)
            and len(cols) > 1
            and (label_cls is None or label_cls == MultiCategoryList)
        ):
    ...
    def databunch(self, **kwargs):
        ...
        raise Exception(
            "Your data is neither split nor labeled, can't turn it into a `DataBunch` yet."
        )
...
class EmptyLabelList(ItemList):
    ...
    def reconstruct(self, t: Tensor, x: Tensor = None):
        if len(t.size()) == 0:
            return EmptyLabel()
        return (
            self.x.reconstruct(t, x) if has_arg(self.x.reconstruct, "x") else self.x.reconstruct(t)
        )
...
class CategoryProcessor(PreProcessor):
    ...
    def process_one(self, item):
        if isinstance(item, EmptyLabel):
            return item
        res = self.c2i.get(item, None)
        if res is None:
            self.warns.append(str(item))
        return res
...
class MultiCategoryList(CategoryListBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if label_delim is not None:
            items = array(csv.reader(items.astype(str), delimiter=label_delim))
        super().__init__(items, classes=classes, **kwargs)
        if one_hot:
    ...
    def get(self, i):
        ...
        if o is None:
            return None
        if self.one_hot:
            return self.reconstruct(o.astype(np.float32))
    ...
    def reconstruct(self, t):
        o = [i for i in range(self.c) if t[i] == 1.0]
...
class ItemLists:
    ...
    def __init__(self, path: PathOrStr, train: ItemList, valid: ItemList):
        ...
        if not self.valid.ignore_empty and len(self.valid.items) == 0:
            warn(
                """Your validation set is empty. If this is by design, use `split_none()`
                 or pass `ignore_empty=True` when labelling to remove this warning."""
            )
        if isinstance(self.train, LabelList):
    ...
    def label_from_lists(
        self, train_labels: Iterator, valid_labels: Iterator, label_cls: Callable = None, **kwargs
    ) -> "LabelList":
        ...
        self.valid = self.valid._label_list(
            x=self.valid, y=self.train.y.new(valid_labels, **kwargs)
        )
        self.__class__ = LabelLists
        self.process()
        return self
    ...
    def transform_y(self, tfms: Optional[Tuple[TfmList, TfmList]] = (None, None), **kwargs):
        ...
        if self.test:
            self.test.transform_y(tfms[1], **kwargs)
        return self
...
class LabelLists(ItemLists):
    ...
    def add_test(self, items: Iterator, label: Any = None, tfms=None, tfm_y=None):
        ...
        if isinstance(items, MixedItemList):
            items = self.valid.x.new(items.item_lists, inner_df=items.inner_df).process()
        elif isinstance(items, ItemList):
            items = self.valid.x.new(items.items, inner_df=items.inner_df).process()
        else:
            items = self.valid.x.new(items).process()
...
def _check_kwargs(ds: ItemList, tfms: TfmList, **kwargs):
    tfms = listify(tfms)
    if (tfms is None or len(tfms) == 0) and len(kwargs) == 0:
        return
    if len(ds.items) >= 1:
        x = ds[0]
        try:
            x.apply_tfms(tfms, **kwargs)
...
class LabelList(Dataset):
    ...
    def __len__(self) -> int:
        return len(self.x) if self.item is None else 1
    ...
    def new(self, x, y, tfms=None, tfm_y=None, **kwargs) -> "LabelList":
        ...
        if isinstance(x, ItemList):
            ...
            return self.new(
                self.x.new(x, **kwargs), self.y.new(y, **kwargs), tfms=tfms, tfm_y=tfm_y
            ).process()

    def __getattr__(self, k: str) -> Any:
        x = super().__getattribute__("x")
        res = getattr(x, k, None)
        if res is not None and k not in ["classes", "c"]:
            return res
    ...
    def get_state(self, **kwargs):
        ...
        state = {
            "x_cls": self.x.__class__,
            "x_proc": self.x.processor,
            "y_cls": self.y.__class__,
            "y_proc": self.y.processor,
            "tfms": self.tfms,
            "tfm_y": self.tfm_y,
            "tfmargs": self.tfmargs,
        }
        if hasattr(self, "tfms_y"):
            state["tfms_y"] = self.tfms_y
        if hasattr(self, "tfmargs_y"):
            state["tfmargs_y"] = self.tfmargs_y
        return {**state, **kwargs}
    ...
    def load_state(cls, path: PathOrStr, state: dict) -> "LabelList":
        ...
        if state.get("tfms_y", False):
            res.tfms_y = state["tfms_y"]
        if state.get("tfmargs_y", False):
            res.tfmargs_y = state["tfmargs_y"]
        if state.get("normalize", False):
            res.normalize = state["normalize"]
        return res

    def process(
        self,
        xp: PreProcessor = None,
        yp: PreProcessor = None,
        name: str = None,
        max_warn_items: int = 5,
    ):
            ...
            if filt.sum() > 0:
                ...
                for p in self.y.processor:
                    if len(getattr(p, "warns", [])) > 0:
                        ...
                        if len(warnings) > max_warn_items:
                            self.warn += "..."
                    p.warns = []
    ...
    def transform(self, tfms: TfmList, tfm_y: bool = None, **kwargs):
        ...
        tfms_y = (
            None
            if tfms is None
            else list(filter(lambda t: getattr(t, "use_on_y", True), listify(tfms)))
        )
        if tfm_y:
            _check_kwargs(self.y, tfms_y, **kwargs)
    ...
    def transform_y(self, tfms: TfmList = None, **kwargs):
        ...
        tfms_y = list(
            filter(
                lambda t: getattr(t, "use_on_y", True), listify(self.tfms if tfms is None else tfms)
            )
        )
...
class MixedItemList(ItemList):
    def __init__(
        ...
        if processor is None:
            ...
            processor = MixedProcessor(
                [ifnone(il.processor, dp) for il, dp in zip(item_lists, default_procs)]
            )
        items = range_of(item_lists[0]) if len(item_lists) >= 1 else []
        if path is None and len(item_lists) >= 1:
            path = item_lists[0].path
        super().__init__(
            items,
            processor=processor,
            path=path,
    ...
    def __getitem__(self, idxs: int) -> Any:
        ...
        if isinstance(idxs, Integral):
            ...
            item_lists = [
                il.new(il.items[idxs], inner_df=index_row(il.inner_df, idxs))
                for il in self.item_lists
            ]
