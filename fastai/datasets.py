...
def _url2tgz(url, data=True, ext: str = ".tgz"):
    return (
        datapath4file(f"{url2name(url)}{ext}", ext=ext)
        if data
        else modelpath4file(f"{url2name(url)}{ext}", ext=ext)
    )
...
def untar_data(
    ...
    if not dest.exists():
        ...
        if verbose:
            print(".tgz file downloaded. Extracting the contents...")
