...
class TrackerCallback(LearnerCallback):
    ...
    def __init__(self, learn: Learner, monitor: str = "valid_loss", mode: str = "auto"):
        ...
        mode_dict["auto"] = (
            np.less if "loss" in self.monitor or "error" in self.monitor else np.greater
        )
    ...
    def get_monitor_value(self):
        ...
        if self.monitor == "trn_loss" and len(self.learn.recorder.losses) == 0:
            ...
            return None
        values = {
            "train_loss": self.learn.recorder.losses[-1].cpu().numpy(),
...
class ReduceLROnPlateauCallback(TrackerCallback):
    ...
    def __init__(
        ...
        super().__init__(learn, monitor=monitor, mode=mode)
        self.patience, self.factor, self.min_delta, self.min_lr = (
            patience,
            factor,
            min_delta,
            min_lr,
