...
class RNNTrainer(LearnerCallback):
    ...
    def on_loss_begin(
        self, last_output: Tuple[Tensor, Sequence[Tensor], Sequence[Tensor]], **kwargs
    ):
    ...
    def on_backward_begin(self, last_loss: Rank0Tensor, last_input: Tensor, **kwargs):
        ...
        # AR and TAR
        if self.alpha != 0.0:
            last_loss += self.alpha * self.out[-1].float().pow(2).mean()
        if self.beta != 0.0:
            h = self.raw_out[-1]
            if len(h) > 1:
                last_loss += self.beta * (h[:, 1:] - h[:, :-1]).float().pow(2).mean()
