...
def params_size(
    ...
    if isinstance(m, Learner):
        ...
        x = next(m.parameters()).new(1, *size)
    else:
...
def model_summary(m: Learner, n: int = 70):
    ...
    if "wd" in str(m.opt_func) or "weight_decay" in str(m.opt_func):
        res += f"\x1b[1;31m Specifying weight decay in the optimizer has no effect, Learner will overwrite \x1b[0m \n"
    if "lr" in str(m.opt_func) or "learning_rate" in str(m.opt_func):
        res += f"\x1b[1;31m Specifying lr in the optimizer has no effect, pass it to fit or the defaults.lr will apply \x1b[0m \n"
