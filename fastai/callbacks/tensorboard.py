...
class GANTensorboardWriter(LearnerTensorboardWriter):
    ...
    def _write_images(self, iteration: int) -> None:
        ...
        try:
            ...
            self.img_gen_vis.write(
                learn=self.learn,
                trn_batch=self.trn_batch,
...
class HistogramTBRequest(TBWriteRequest):
    ...
    def __init__(self, model: nn.Module, iteration: int, tbwriter: SummaryWriter, name: str):
        ...
        self.params = [
            (name, values.clone().detach().cpu()) for (name, values) in model.named_parameters()
        ]
        self.name = name
...
class HistogramTBWriter:
    ...
    def write(
        self, model: nn.Module, iteration: int, tbwriter: SummaryWriter, name: str = "model"
    ) -> None:
        "Writes model histograms to Tensorboard."
...
class ModelStatsTBRequest(TBWriteRequest):
    ...
    def __init__(self, model: nn.Module, iteration: int, tbwriter: SummaryWriter, name: str):
        super().__init__(tbwriter=tbwriter, iteration=iteration)
        self.gradients = [
            x.grad.clone().detach().cpu() for x in model.parameters() if x.grad is not None
        ]
        self.name = name
...
class ModelStatsTBWriter:
    ...
    def write(
        self, model: nn.Module, iteration: int, tbwriter: SummaryWriter, name: str = "model_stats"
    ) -> None:
        "Writes model gradient statistics to Tensorboard."
...
class ImageTBRequest(TBWriteRequest):
    ...
    def _write_images(self, name: str, images: [Tensor]) -> None:
        ...
        self.tbwriter.add_image(
            tag=tag, img_tensor=vutils.make_grid(images, normalize=True), global_step=self.iteration
        )
...
class ImageTBWriter:
    ...
    def _write_for_dstype(
        ...
        request = ImageTBRequest(
            learn=learn, batch=batch, iteration=iteration, tbwriter=tbwriter, ds_type=ds_type
        )
        asyncTBWriter.request_write(request)
