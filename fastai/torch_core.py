...
fastai_types = {
    ...
    ListRules: "ListRules",
    ListSizes: "ListSizes",
    NPArrayableList: "NPArrayableList",
    NPArrayList: "NPArrayList",
    NPArrayMask: "NPArrayMask",
    NPImage: "NPImage",
    OptDataFrame: "OptDataFrame",
    OptListOrItem: "OptListOrItem",
    OptRange: "OptRange",
    OptStrTuple: "OptStrTuple",
    OptStats: "OptStats",
    PathOrStr: "PathOrStr",
    PBar: "PBar",
    Point: "Point",
    Points: "Points",
    Sizes: "Sizes",
    SplitArrayList: "SplitArrayList",
    StartOptEnd: "StartOptEnd",
    StrList: "StrList",
    Tokens: "Tokens",
    OptStrList: "OptStrList",
    AffineMatrix: "AffineMatrix",
    BoolOrTensor: "BoolOrTensor",
    FloatOrTensor: "FloatOrTensor",
    IntOrTensor: "IntOrTensor",
    ItemsList: "ItemsList",
    LambdaFunc: "LambdaFunc",
    LayerFunc: "LayerFunc",
    ModuleList: "ModuleList",
    OptOptimizer: "OptOptimizer",
    ParamList: "ParamList",
    Rank0Tensor: "Rank0Tensor",
    SplitFunc: "SplitFunc",
    SplitFuncOrIdxList: "SplitFuncOrIdxList",
    TensorOrNumber: "TensorOrNumber",
    TensorOrNumList: "TensorOrNumList",
    ...
    HookFunc: "HookFunc",
    LogitTensorImage: "LogitTensorImage",
    LossFunction: "LossFunction",
    MetricFunc: "MetricFunc",
    MetricFuncList: "MetricFuncList",
    MetricsList: "MetricsList",
    OptLossFunc: "OptLossFunc",
    OptMetrics: "OptMetrics",
    OptSplitFunc: "OptSplitFunc",
    PixelFunc: "PixelFunc",
    LightingFunc: "LightingFunc",
    IntsOrStrs: "IntsOrStrs",
...
def to_half(b: Collection[Tensor]) -> Collection[Tensor]:
    ...
    return recurse(
        lambda x: x.half() if x.dtype not in [torch.int64, torch.int32, torch.int16] else x, b
    )
...
def to_float(b: Collection[Tensor]) -> Collection[Tensor]:
    "Recursively map lists of tensors in `b ` to FP16."
    return recurse(
        lambda x: x.float() if x.dtype not in [torch.int64, torch.int32, torch.int16] else x, b
    )
...
flatten_model = (
    lambda m: sum(map(flatten_model, children_and_parameters(m)), []) if num_children(m) else [m]
)
...
class NoneReduceOnCPU:
    ...
    def __exit__(self, type, value, traceback):
        if self.device is not None:
            self.loss_func.weight = self.loss_func.weight.to(self.device)
...
def try_int(o: Any) -> Any:
    ...
    if isinstance(o, Sized) or getattr(o, "__array_interface__", False):
        return o
    try:
        return int(o)
    except:
...
def add_metrics(
    last_metrics: Collection[Rank0Tensor], mets: Union[Rank0Tensor, Collection[Rank0Tensor]]
):
