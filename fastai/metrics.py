...
class KappaScore(ConfusionMatrix):
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        ...
        if self.weights is None:
            ...
            raise ValueError('Unknown weights. Expected None, "linear", or "quadratic".')
...
class MultiLabelFbeta(Callback):
    ...
    def __init__(self, beta=2, eps=1e-15, thresh=0.3, sigmoid=True, average="micro"):
        self.eps, self.thresh, self.sigmoid, self.average, self.beta = (
            eps,
            thresh,
            sigmoid,
            average,
    ...
    def on_batch_end(self, last_output, last_target, **kwargs):
        pred, targ = (
            ((last_output.sigmoid() if self.sigmoid else last_output) > self.thresh).byte(),
            last_target.byte(),
        )
        m = pred * targ
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        ...
        if self.average == "micro":
            precision, recall = (
                self.tp.sum() / self.total_pred.sum(),
                self.tp.sum() / self.total_targ.sum(),
            )
            ...
            res = listify(
                self.fbeta_score((self.tp / self.total_pred), (self.tp / self.total_targ))
            )
        else:
