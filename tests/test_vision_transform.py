...
def bbox2pic(corners, size):
    ...
    pic[0, corners[0] : corners[2], corners[1] : corners[3]] = 1.0
    return Image(pic)
...
def points2pic(points, size):
    ...
    for p in points:
        pic[
            0,
            max(0, int(p[0]) - 1) : min(size, int(p[0]) + 1),
            max(0, int(p[1]) - 1) : min(size, int(p[1]) + 1),
        ] = 1.0
    return Image(pic)
...
def test_points_data_aug():
    ...
    for p in new_pnts.round():
        if (
            tfm_x.data[
                0,
                max(0, int(p[0]) - 1) : min(int(p[0]) + 2, 64),
                max(0, int(p[1]) - 1) : min(int(p[1]) + 2, 64),
            ].sum()
            < 0.8
        ):
            fail = True
...
def test_mask_data_aug():
    ...
    assert (new_mask.float() - tfm_y.data[0].float()).sum() < 1.0


def img_test(cs):
    points = torch.zeros(5, 5)
    if not is_listy(cs[0]):
        cs = [cs]
    for c in cs:
        points[c[0], c[1]] = 1
...
def test_deterministic_transforms():
    ...
    check_tfms(
        img,
        [rotate(degrees=90), rotate(degrees=-90), flip_lr(), flip_affine()],
        [[1, 3], [3, 1], [3, 1], [3, 1]],
    )
    check_tfms(
        img,
        [zoom(scale=2), squish(scale=0.5), squish(scale=2)],
        [[4, 4], [3, 4], [4, 3]],
        mode="nearest",
    )
    crops = [
        crop(size=4, row_pct=r, col_pct=c)
        for r, c in zip([0.0, 0.0, 0.5, 0.99, 0.99], [0.0, 0.99, 0.5, 0.0, 0.99])
    ]
...
def test_crops_with_tensor_image_sizes():
    ...
    img = img_test([3, 3])
    crops = [
        crop(size=(1, 4, 4), row_pct=r, col_pct=c)
        for r, c in zip([0.0, 0.0, 0.5, 0.99, 0.99], [0.0, 0.99, 0.5, 0.0, 0.99])
    ]
    check_tfms(img, crops, [[3, 3], [3, 2], [2, 2], [2, 3], [2, 2]])
