...
def learn():
    ...
    data = (
        TextList.from_df(df, path, cols="texts")
        .split_by_idx(list(range(len(df_trn), len(df))))
        .label_for_lm()
        .add_test(df["texts"].iloc[:200].values)
        .databunch()
    )
    learn = language_model_learner(data, AWD_LSTM, pretrained=False, config=config(), drop_mult=0.0)
...
def n_params(learn):
    return sum([len(pg["params"]) for pg in learn.opt.opt.param_groups])
...
def test_qrnn_works_with_no_split():
    ...
    learn = language_model_learner(
        data, AWD_LSTM, pretrained=False, config=config(True), drop_mult=0.0
    )
...
def test_mem_leak():
    ...
    assert (
        gc_collected < 102
    )  # scipy has some cyclic references that we want to ignore (this accounts for 100 objects).
...
def test_order_preds():
    ...
    data_clas = (
        TextList.from_df(df_val, path, cols="texts")
