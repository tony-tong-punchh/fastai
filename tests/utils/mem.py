...
def gpu_mem_consume_some(n):
    return torch.ones((n, n)).cuda()


def gpu_mem_consume_16mb():
    return gpu_mem_consume_some(2000)
...
def check_mem(used_exp, peaked_exp, used_rcv, peaked_rcv, abs_tol=2, ctx=None):
    ...
    assert isclose(
        used_exp, used_rcv, abs_tol=abs_tol
    ), f"used mem: expected={used_exp} received={used_rcv}{ctx}"
    assert isclose(
        peaked_exp, peaked_rcv, abs_tol=abs_tol
    ), f"peaked mem: expected={peaked_exp} received={peaked_rcv}{ctx}"
