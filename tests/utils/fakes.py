...
class RandomItem(ItemBase):
    ...
    def __init__(self, *sizes):
        self.obj, self.data = sizes, torch.randn(*sizes)

    def __str__(self):
        return f"Size {self.obj}\n{self.data}"
...
class RandomLabel(ItemBase):
    ...
    def __init__(self, y_range):
        self.obj, self.data = y_range, torch.randint(*y_range, ())
...
class RandomLabelList(ItemList):
    ...
    def get(self, i):
        return RandomLabel(self.y_range)

    def reconstruct(self, t):
        return RandomLabel.from_val(t)
...
class RandomItemList(ItemList):
    ...
    def get(self, i):
        return RandomItem(*self.sizes)

    def reconstruct(self, t):
        return RandomItem.from_val(t)
...
def fake_basedata(
    n_in: int = 5, batch_size: int = 5, train_length: int = None, valid_length: int = None
):
    if train_length is None:
        train_length = 2 * batch_size
...
def fake_data(
    ...
    if valid_length is None:
        valid_length = batch_size
    return (
        RandomItemList([0] * (train_length + valid_length), sizes=[n_in])
        .split_by_idx(list(range(valid_length)))
        .label_const(0.0, y_range=[0, n_out])
