...
def text_files(path, labels):
    ...
    for lbl in labels:
        ...
        for i, t in enumerate(texts):
            with open(path / "temp" / lbl / f"{lbl}_{i}.txt", "w") as f:
                f.write(t)
...
def test_from_folder():
    ...
    data = (
        TextList.from_folder(path / "temp").split_by_rand_pct(0.1).label_from_folder().databunch()
    )
...
def test_filter_classes():
    ...
    with pytest.warns(UserWarning):
        data = (
            TextList.from_folder(path / "temp")
...
def special_fastai_test_rule(s):
    return s.replace("fast ai", "@fastdotai")
...
def test_from_csv_and_from_df():
    ...
    data1 = TextClasDataBunch.from_df(
        path,
        train_df=trn_df,
        valid_df=val_df,
        test_df=tst_df,
    ...
    x, y = next(
        iter(data1.valid_dl)
    )  # Will fail if the SortSampler keys get messed up between train and valid.
    df = text_df(["neg", "pos", "neg pos"])
    data2 = TextClasDataBunch.from_df(
        path,
        train_df=trn_df,
        valid_df=val_df,
        label_cols=0,
    ...
    assert len(y.data) == 2
    assert (
        "@fastdotai" in data2.train_ds.vocab.itos
    ), "custom tokenzier not used by TextClasDataBunch"
    text_csv_file(path / "tmp.csv", ["neg", "pos"])
    data3 = TextLMDataBunch.from_csv(
        path, "tmp.csv", test="tmp.csv", label_cols=0, text_cols=["text"], bs=2
    )
    assert isinstance(data3.train_ds.y[0], EmptyLabel)
    data4 = TextLMDataBunch.from_csv(
        path, "tmp.csv", label_cols=0, text_cols=["text"], max_vocab=5, bs=2
    )
...
def test_should_load_backwards_lm_1():
    ...
    data = TextLMDataBunch.from_df(
        path, train_df=df, valid_df=df, label_cols=0, text_cols=["text"], bs=2, backwards=False
    )
    this_tests(data.one_batch)
    batch_forward = data.one_batch(DatasetType.Valid)[0].numpy()

    data = TextLMDataBunch.from_df(
        path, train_df=df, valid_df=df, label_cols=0, text_cols=["text"], bs=2, backwards=True
    )
...
def test_backwards_cls_databunch():
    ...
    data = TextClasDataBunch.from_df(
        path, train_df=df, valid_df=df, label_cols=0, text_cols=["text"], bs=4, backwards=True
    )
    this_tests(data.one_batch)
    ...
    for ds in [DatasetType.Train, DatasetType.Valid]:
        ...
        for sample in batch[0]:
            ...
            assert any(
                [orig in as_text for orig in orig_texts]
            )  # batch samples contain BOS and optionally PAD tokens
...
def test_load_and_save_test():
    ...
    df = text_df(["neg", "pos"])
    data = TextClasDataBunch.from_df(
        path, train_df=df, valid_df=df, test_df=df, label_cols=0, text_cols="text", bs=10
    )
    data.save()
...
def test_from_ids_works_for_equally_length_sentences():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        data = TextClasDataBunch.from_ids(
            tmp,
            vocab=Vocab({0: BOS, 1: PAD}),
            train_ids=ids,
            train_lbls=lbl,
            valid_ids=ids,
            valid_lbls=lbl,
...
def test_regression():
    ...
    df = text_df([0.0, 1.0])
    data = (
        TextList.from_df(df, path, cols="text")
