...
def jaggedWithConsecutiveNumbers(bs, sentence_len, iterations, minTokens):
    ...
    while count < total:
        nb = (
            total - count
            if total - count < sentence_len
            else minTokens + int(np.random.random() * sentence_len)
        )
...
def verify_datadirection(
    bs, seq_len, sentence_len, iterations, minTokens, backwards=False, nbTests=1000
):
    for i in range(nbTests):
        jagged, countTokens = jaggedWithConsecutiveNumbers(bs, sentence_len, iterations, minTokens)

        trainIDS = validIDS = jagged
        db = TextLMDataBunch.from_ids(
            ".", None, trainIDS, validIDS, bptt=seq_len, bs=bs, no_check=True
        )
        data = LanguageModelPreLoader(
            db.train_ds, bs=bs, bptt=seq_len, backwards=backwards, shuffle=False
        )
        ...
        assert countIte == len(
            dl
        ), f"number of iteration does not match: countIte:{countIte}!= len(data):{len(dl)} "
        ...
        assert np.all(
            batches[ix - 1, -1] + diff == batches[ix, 0]
        ), f"last token i row-1 {batches[ix-1,-1]}+{diff} must be equal to first element in row:{batches[ix,0]}"
...
def test_backwards_minibatch():
    ...
    minTokens = 1
    verify_datadirection(
        bs, seq_len, sentence_len, iterations, minTokens, backwards=True, nbTests=1000
    )
