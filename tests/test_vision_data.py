...
def test_from_csv_and_from_df(path):
    ...
    for func in ["from_csv", "from_df"]:
        files = []
        if func == "from_df":
            data = ImageDataBunch.from_df(path, df=pd.read_csv(path / "labels.csv"), size=28)
        else:
            data = ImageDataBunch.from_csv(path, size=28)
        mnist_tiny_sanity_test(data)
...
def check_resized(data, size, args):
    ...
    assert (
        size_want == size_real
    ), f"[{args}]: size mismatch after resize {size} expected {size_want}, got {size_real}"
            ...
            for rm_name in rms:
                ...
                with CaptureStderr() as cs:
                    data = ImageDataBunch.from_name_re(
                        p, fnames, pat, ds_tfms=None, size=size, resize_method=rm
                    )
...
def test_trunc_download():
    ...
    with responses.RequestsMock() as rsps:
        ...
        try:
            ...
            assert (
                False
            ), f"untar_data({URLs.COCO_TINY})  should have gracefully failed on a truncated download"
        finally:
...
def test_multi():
    ...
    data = (
        ImageList.from_csv(path, "labels.csv", folder="train", suffix=".jpg")
...
def test_points():
    ...
    data = (
        PointsItemList.from_folder(coco).split_by_rand_pct().label_from_func(get_y_func).databunch()
    )
    _check_data(data, 160, 40)
...
def test_coco():
    ...
    data = (
        ObjectItemList.from_folder(coco)
        .split_by_rand_pct()
