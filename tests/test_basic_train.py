...
def subtest_destroy_mem(data):
    ...
    with GPUMemTrace(on_exit_report=False) as mtrace:
        with CaptureStdout() as cs:
            learn.destroy()
    check_mtrace(
        used_exp=-load_used, peaked_exp=-load_peaked, mtrace=mtrace, abs_tol=10, ctx="destroy"
    )
...
def test_memory(data):
    ...
    if dev_name != "GeForce GTX 1070 Ti":
        pytest.skip(
            f"currently only matched for mem usage on specific GPU models, {dev_name} is not one of them"
        )
...
def test_export_load_learner():
    ...
    for should_destroy in [False, True]:
        ...
        with CaptureStdout() as cs:
            learn.export(destroy=should_destroy)
...
def test_model_load_mem_leak():
    ...
    assert isclose(
        used_before, used_after, abs_tol=6
    ), f"load() and used GPU RAM: before load(): {used_before}, after: {used_after}"
    ...
    assert isclose(
        used_before, used_after_reclaimed, abs_tol=6
    ), f"load() and used GPU RAM: before load(): {used_before}, after: {used_after}, after gc.collect() {used_after_reclaimed} used"
