...
def test_categorify():
    ...
    cat_names = ["A"]
    cont_names = ["X"]
    train_df = pd.DataFrame({"A": ["a", "b"], "X": [0.0, 1.0]})
    valid_df = pd.DataFrame({"A": ["b", "a"], "X": [1.0, 0.0]})
    ...
    assert all([isinstance(dt, CategoricalDtype) for dt in train_df[cat_names].dtypes])
    assert all([isinstance(dt, CategoricalDtype) for dt in valid_df[cat_names].dtypes])
...
def test_fill_missing_leaves_no_na_values():
    cont_names = ["A"]
    train_df = pd.DataFrame({"A": [0.0, np.nan, np.nan]})
    valid_df = pd.DataFrame({"A": [np.nan, 0.0, np.nan]})
...
def test_fill_missing_returns_correct_medians():
    train_df = pd.DataFrame({"A": [0.0, 1.0, 1.0, np.nan, np.nan]})
    valid_df = pd.DataFrame({"A": [0.0, 0.0, 1.0, np.nan, np.nan]})
    expected_filled_train_df = pd.DataFrame({"A": [0.0, 1.0, 1.0, 1.0, 1.0]})
    expected_filled_valid_df = pd.DataFrame({"A": [0.0, 0.0, 1.0, 1.0, 1.0]})
...
def test_cont_cat_split():
    ...
    cat_names = ["A"]
    cont_names = ["F", "I"]
    df = pd.DataFrame({"A": ["a", "b"], "F": [0.0, 1.0], "I": [0, 1]})
...
def test_add_datepart():
    ...
    df = pd.DataFrame(
        {
            "transaction_date": ["02/03/2017", "02/04/2017", "02/05/2017"],
            "category": ["a", "b", "a"],
        }
    )
