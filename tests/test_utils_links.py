...
def assert_link(docstr, expected, nb_cells=None, modules=None, msg=""):
    if modules is None:
        modules = gen_notebooks.get_imported_modules(nb_cells or [])
...
def test_fastai_prefix():
    ...
    assert_link(
        docstr, expected, msg="Should match keywords prefixed with fastai. See `index.ipynb`"
    )
...
def test_link_typedef():
    ...
    assert_link(
        ...
        expected,
        modules=[torch],
        msg="Type definitions to torch formatted incorrectly. See fastai_typing.ipynb",
    )
...
### [`learner`](/text.learner.html#text.learner)"""
    assert_link(
        docstr,
        expected,
        msg="data, models should link to highest module. transform and learner links to first match",
    )
...
def test_link_vision_learner_priority():
    ...
    expected = (
        "Pass in your [`data`](/vision.data.html#vision.data), calculated `preds`, actual `y`,"
    )
    err_msg = "`data` should link to vision.data instead of text.data."
    modules = gen_notebooks.get_imported_modules(
        [gen_notebooks.get_code_cell(imports)], nb_module_name="fastai.vision.learner"
    )
