...
def test_apply_init():
    ...
    assert conv1_w.equal(m[0][0].weight), (
        "Expected first colvulition layer's weights to be %r" % conv1_w
    )
...
def test_to_detach():
    ...
    assert ta.is_cuda
    assert isinstance(
        ta, (torch.cuda.FloatTensor or torch.cuda.DoubleTensor or torch.cuda.HalfTensor)
    )
    assert ta.requires_grad
    assert dtta.is_cuda
    assert isinstance(
        dtta, (torch.cuda.FloatTensor or torch.cuda.DoubleTensor or torch.cuda.HalfTensor)
    )
...
def test_none_reduce_on_cpu():
    ...
    with NoneReduceOnCPU(F.cross_entropy) as lf:
        loss = lf(y_pred, y_true)
        assert isclose(
            loss.sum(), 6.23, abs_tol=1e-2
        ), "final loss without reduction does not seem to be correct"
