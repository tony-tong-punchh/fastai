...
def test_filter_by_rand():
    ...
    assert (
        sd1.items == sd2.items
    ).all(), "Identically seeded random data sets contain different values"


def test_split_subsets():
    this_tests(ItemList.split_subsets)
    sd = ItemList(range(10)).split_subsets(train_size=0.1, valid_size=0.2).label_const(0)
    assert len(sd.train) == 1
    assert len(sd.valid) == 2

    with pytest.raises(AssertionError):
        ItemList(range(10)).split_subsets(train_size=0.6, valid_size=0.6).label_const(0)
    with pytest.raises(AssertionError):
...
def test_regression():
    ...
    data = (
        ItemList.from_df(df, path=".", cols=0).split_by_rand_pct().label_from_df(cols=1).databunch()
    )
    assert data.c == 1
...
def test_filter_by_folder():
    ...
    res = ItemList.filter_by_folder(
        ...
        include=["in", "and_in"],
        exclude=["out", "also_out"],
    )
    assert res.items == [Path("parent/in")]

    res = ItemList.filter_by_folder(
        ItemList(items=items), include=["in", "and_in"], exclude=["out", "also_out"]
    )
