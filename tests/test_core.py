...
def test_recurse():
    ...
    def to_plus(x, a=1):
        return recurse(lambda x, a: x + a, x, a)
    ...
    assert to_plus({"a": 1, "b": [1, 2, 3], "c": {"d": 4, "e": 5}}) == {
        "a": 2,
        "b": [2, 3, 4],
        "c": {"d": 5, "e": 6},
    }
...
def test_find_classes():
    ...
    try:
        assert [o.name for o in find_classes(path)] == classes
    finally:
        shutil.rmtree(path)
...
def test_arrays_split():
    ...
    with pytest.raises(Exception):
        arrays_split([0, 5], [1, 2, 3, 4, 5])
    with pytest.raises(Exception):
        arrays_split([0, 3], [1, 2, 3, 4, 5], [1, 2, 3, 4])
...
def test_random_split():
    ...
    with pytest.raises(Exception):
        random_split(1.1, [1, 2, 3])
    with pytest.raises(Exception):
        random_split(0.1, [1, 2, 3], [1, 2, 3, 4])
