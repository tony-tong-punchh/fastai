...
def get_language_model(n_tok, em_sz, nhid, nlayers, pad_token, decode_train=True, dropouts=None):
    ...
    rnn_dec = LinearDecoder(
        n_tok, em_sz, dropouts[1], decode_train=decode_train, tie_encoder=rnn_enc.encoder
    )
...
class CrossEntDecoder(nn.Module):
    ...
    def get_rand_idxs(self):
        return pt_sample(self.prs, self.n_neg)
...
def get_learner(drops, n_neg, sampled, md, em_sz, nh, nl, opt_fn, prs):
    m = to_gpu(
        get_language_model(md.n_tok, em_sz, nh, nl, md.pad_idx, decode_train=False, dropouts=drops)
    )
