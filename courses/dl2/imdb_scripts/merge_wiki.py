...
def write_file(file_path, text_iter, num_tokens):
    ...
    print("{}. # documents: {:,}. # tokens: {:,}.".format(file_path, j, total_num_tokens))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--output",
        required=True,
        help="the output directory where the merged Wikipedia " "documents should be saved",
    )
    parser.add_argument(
        "-n",
        "--num-tokens",
        type=int,
        default=100000000,
        help="the # of tokens that the merged document should " "contain (default: 100M)",
    )
