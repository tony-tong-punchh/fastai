...
def rf_feat_importance(m, df):
    return pd.DataFrame({"cols": df.columns, "imp": m.feature_importances_}).sort_values(
        "imp", ascending=False
    )
...
def set_rf_samples(n):
    ...
    n random rows.
    """
    forest._generate_sample_indices = lambda rs, n_samples: forest.check_random_state(rs).randint(
        0, n_samples, n
    )
