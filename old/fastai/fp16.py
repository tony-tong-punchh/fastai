...
def copy_model_to_fp32(m, optim):
    ...
    """
    fp32_params = [
        m_param.clone().type(torch.cuda.FloatTensor).detach() for m_param in trainable_params_(m)
    ]
    ...
    for group_params in optim_groups:
        for i in range(len(group_params)):
            if not group_params[i].requires_grad:
                continue  # only update trainable_params_
