...
def resize_imgs(fnames, targ, path, new_path, resume=True, fn=None):
    ...
    if resume:
        ...
        for subdir in subdirs:
            files = [
                os.path.join(subdir, file) for file in os.listdir(os.path.join(target_path, subdir))
            ]
...
def read_dir(path, folder):
    ...
    if any(fnames):
        ...
        raise FileNotFoundError(
            "{} has subdirectories but contains no files. Is your directory structure is correct?".format(
                full_path
            )
        )
    else:
...
def nhot_labels(label2idx, csv_labels, fnames, c):
    ...
    all_idx = {
        k: n_hot([label2idx[o] for o in ([] if type(v) == float else v)], c)
...
def dict_source(folder, fnames, csv_labels, suffix="", continuous=False):
    all_labels = sorted(
        list(set(p for o in csv_labels.values() for p in ([] if type(o) == float else o)))
    )
...
def open_image(fn):
    ...
    if not os.path.exists(fn) and not str(fn).startswith("http"):
        ...
        if slice.PhotometricInterpretation.startswith("MONOCHROME"):
            ...
            raise OSError(
                "Unsupported DICOM image with PhotometricInterpretation=={}".format(
                    slice.PhotometricInterpretation
                )
            )
    else:
...
class ImageData(ModelData):
    ...
    def get_dl(self, ds, shuffle):
        if ds is None:
            return None
        return DataLoader(
            ds, batch_size=self.bs, shuffle=shuffle, num_workers=self.num_workers, pin_memory=False
        )
    ...
    def resize(self, targ_sz, new_path="tmp", resume=True, fn=None):
        ...
        t = tqdm_notebook(dls)
        for dl in t:
            new_ds.append(self.resized(dl, targ_sz, new_path, resume, fn))
        t.close()
...
class ImageClassifierData(ImageData):
    ...
    def from_path_and_array(
        ...
        return cls.from_names_and_array(
            path, fnames, y, classes, val_idxs, test_name, num_workers=num_workers, tfms=tfms, bs=bs
        )
    ...
    def from_names_and_array(
        ...
        datasets = cls.get_ds(
            f, (trn_fnames, trn_y), (val_fnames, val_y), tfms, path=path, test=test_fnames
        )
        return cls(path, datasets, bs, num_workers, classes=classes)
