...
def zoom_cv(x, z):
    ...
    if z == 0:
        return x
    r, c, *_ = x.shape
    M = cv2.getRotationMatrix2D((c / 2, r / 2), 0, z + 1.0)
...
def lighting(im, b, c):
    ...
    if b == 0 and c == 1:
        return im
    mu = np.average(im)
    return np.clip((im - mu) * c + mu + b, 0.0, 1.0).astype(np.float32)
...
def det_rotate(deg):
    return lambda x: rotate_cv(x, deg)
...
class NoCrop(CoordTransform):
    ...
    def do_transform(self, x, is_y):
        if is_y:
            return no_crop(
                x, self.sz_y, cv2.INTER_AREA if self.tfm_y == TfmType.PIXEL else cv2.INTER_NEAREST
            )
        else:
...
class RandomLighting(Transform):
    ...
    def do_transform(self, x, is_y):
        if is_y and self.tfm_y != TfmType.PIXEL:
            return x
        b = self.store.b_rand
        c = self.store.c_rand
...
class RandomRotateZoom(CoordTransform):
    ...
    def __init__(self, deg, zoom, stretch, ps=None, mode=cv2.BORDER_REFLECT, tfm_y=TfmType.NO):
        ...
        self.transforms = (
            RandomRotate(deg, p=1, mode=mode, tfm_y=tfm_y),
            RandomZoom(zoom, tfm_y=tfm_y),
            RandomStretch(stretch, tfm_y=tfm_y),
        )
...
class RandomBlur(Transform):
    ...
    def do_transform(self, x, is_y):
        return (
            cv2.GaussianBlur(src=x, ksize=self.store.kernel, sigmaX=0)
...
class GoogleNetResize(CoordTransform):
    ...
    def do_transform(self, x, is_y):
        ...
        if is_y:
            interpolation = (
                cv2.INTER_NEAREST
                if self.tfm_y in (TfmType.COORD, TfmType.CLASS)
                else cv2.INTER_AREA
            )
        else:
            interpolation = cv2.INTER_AREA
...
crop_fn_lu = {
    ...
    CropType.NOOP: CropNoop,
}


class Transforms:
    def __init__(
        self, sz, tfms, normalizer, denorm, crop_type=CropType.CENTER, tfm_y=TfmType.NO, sz_y=None
    ):
        if sz_y is None:
            sz_y = sz
...
def tfms_from_stats(
    ...
    if aug_tfms is None:
        aug_tfms = []
    tfm_norm = (
        Normalize(*stats, tfm_y=tfm_y if norm_y else TfmType.NO) if stats is not None else None
    )
    ...
    val_tfm = image_gen(
        tfm_norm, tfm_denorm, sz, pad=pad, crop_type=val_crop, tfm_y=tfm_y, sz_y=sz_y, scale=scale
    )
    trn_tfm = image_gen(
        tfm_norm,
        tfm_denorm,
