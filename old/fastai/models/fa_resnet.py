...
def load(model, pretrained, name):
    if pretrained:
        model.load_state_dict(
            updated_dict(model_zoo.load_url(model_urls[name]), model.state_dict())
        )
    return model
...
def fa_resnet18(pretrained=False, **kwargs):
    return load(ResNet(BasicBlock, [2, 2, 2, 2], **kwargs), pretrained, "resnet18")


def fa_resnet34(pretrained=False, **kwargs):
    return load(ResNet(BasicBlock, [3, 4, 6, 3], **kwargs), pretrained, "resnet34")


def fa_resnet50(pretrained=False, **kwargs):
    return load(ResNet(Bottleneck, [3, 4, 6, 3], **kwargs), pretrained, "resnet50")


def fa_resnet101(pretrained=False, **kwargs):
    return load(ResNet(Bottleneck, [3, 4, 23, 3], **kwargs), pretrained, "resnet101")


def fa_resnet152(pretrained=False, **kwargs):
    return load(ResNet(Bottleneck, [3, 8, 36, 3], **kwargs), pretrained, "resnet152")
...
def vgg_resnet50():
    return ResNet(Bottleneck, [3, 4, 6, 3], vgg_head=True)
