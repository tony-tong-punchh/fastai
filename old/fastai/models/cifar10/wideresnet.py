...
class WideResNet(nn.Module):
    def __init__(self, num_groups, N, num_classes, k=1, drop_p=0.0, start_nf=16):
        ...
        for i in range(num_groups):
            layers += _make_group(
                N, n_channels[i], n_channels[i + 1], BasicBlock, (1 if i == 0 else 2), drop_p
            )
...
def wrn_22_k8():
    return WideResNet(num_groups=3, N=3, num_classes=10, k=8, drop_p=0.0)


def wrn_22_k10():
    return WideResNet(num_groups=3, N=3, num_classes=10, k=10, drop_p=0.0)


def wrn_22_k8_p2():
    return WideResNet(num_groups=3, N=3, num_classes=10, k=8, drop_p=0.2)


def wrn_28():
    return WideResNet(num_groups=3, N=4, num_classes=10, k=6, drop_p=0.0)


def wrn_28_k8():
    return WideResNet(num_groups=3, N=4, num_classes=10, k=8, drop_p=0.0)
