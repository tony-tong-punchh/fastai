...
class ColumnarModelData(ModelData):
    def __init__(self, path, trn_ds, val_ds, bs, test_ds=None, shuffle=True):
        test_dl = (
            DataLoader(test_ds, bs, shuffle=False, num_workers=1) if test_ds is not None else None
        )
        super().__init__(
            path,
            DataLoader(trn_ds, bs, shuffle=shuffle, num_workers=1),
    ...
    @classmethod
    def from_arrays(
        cls, path, val_idxs, xs, y, is_reg=True, is_multi=False, bs=64, test_xs=None, shuffle=True
    ):
        ((val_xs, trn_xs), (val_y, trn_y)) = split_by_idx(val_idxs, xs, y)
        test_ds = (
            PassthruDataset(*(test_xs.T), [0] * len(test_xs), is_reg=is_reg, is_multi=is_multi)
            if test_xs is not None
            else None
        )
        return cls(
            path,
            PassthruDataset(*(trn_xs.T), trn_y, is_reg=is_reg, is_multi=is_multi),
    ...
    def from_data_frames(
        ...
        test_ds = (
            ColumnarDataset.from_data_frame(test_df, cat_flds, None, is_reg, is_multi)
    ...
    def get_learner(
        self, emb_szs, n_cont, emb_drop, out_sz, szs, drops, y_range=None, use_bn=False, **kwargs
    ):
...
class StructuredLearner(Learner):
    ...
    def _get_crit(self, data):
        return (
            F.mse_loss if data.is_reg else F.binary_cross_entropy if data.is_multi else F.nll_loss
        )
    ...
    def summary(self):
        x = [
            torch.ones(3, self.data.trn_ds.cats.shape[1]).long(),
            torch.rand(3, self.data.trn_ds.conts.shape[1]),
        ]
...
class CollabFilterDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return [o[idx] for o in self.cols]
    ...
    def get_model(self, n_factors):
        model = EmbeddingDotBias(
            n_factors, self.n_users, self.n_items, self.min_score, self.max_score
        )
...
class EmbeddingDotBias(nn.Module):
    def __init__(self, n_factors, n_users, n_items, min_score, max_score):
        ...
        (self.u, self.i, self.ub, self.ib) = [
            get_emb(*o)
            for o in [(n_users, n_factors), (n_items, n_factors), (n_users, 1), (n_items, 1)]
        ]
...
class CollabFilterLearner(Learner):
    ...
    def summary(self):
        return model_summary(self.model, [torch.ones(3).long(), torch.ones(3).long()])
