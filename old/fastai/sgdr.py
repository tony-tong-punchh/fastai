...
class LR_Finder2(LR_Finder):
    ...
    def plot(self, n_skip=10, n_skip_end=5, smoothed=True):
        ...
        for i, m in enumerate(self.metrics):
            ...
            if smoothed:
                values = smooth_curve(values, 0.98)
...
class DecayScheduler:
    ...
    def __init__(self, dec_type, num_it, start_val, end_val=None, extra=None):
        self.dec_type, self.nb, self.start_val, self.end_val, self.extra = (
            dec_type,
            num_it,
            start_val,
            end_val,
    ...
    def next_val(self):
        ...
        if self.dec_type == DecayType.NO:
            ...
            return (
                self.end_val
                + (self.start_val - self.end_val) * (1 - self.it / self.nb) ** self.extra
            )
...
class TrainingPhase:
    ...
    def __init__(
        ...
        """
        self.epochs, self.opt_fn, self.lr, self.momentum, self.beta, self.wds = (
            epochs,
            opt_fn,
            lr,
            momentum,
        ...
        if isinstance(lr_decay, tuple):
            self.lr_decay, self.extra_lr = lr_decay
        else:
            self.lr_decay, self.extra_lr = lr_decay, None
        if isinstance(momentum_decay, tuple):
            self.mom_decay, self.extra_mom = momentum_decay
        else:
            self.mom_decay, self.extra_mom = momentum_decay, None
        self.wd_loss = wd_loss

    def phase_begin(self, layer_opt, nb_batches):
        self.layer_opt = layer_opt
        if isinstance(self.lr, tuple):
            start_lr, end_lr = self.lr
        else:
            start_lr, end_lr = self.lr, None
        self.lr_sched = DecayScheduler(
            self.lr_decay, nb_batches * self.epochs, start_lr, end_lr, extra=self.extra_lr
        )
        if isinstance(self.momentum, tuple):
            start_mom, end_mom = self.momentum
        else:
            start_mom, end_mom = self.momentum, None
        self.mom_sched = DecayScheduler(
            self.mom_decay, nb_batches * self.epochs, start_mom, end_mom, extra=self.extra_mom
        )
        self.layer_opt.set_opt_fn(self.opt_fn)
        self.layer_opt.set_lrs(start_lr)
        self.layer_opt.set_mom(start_mom)
        if self.beta is not None:
            self.layer_opt.set_beta(self.beta)
        if self.wds is not None:
            if self.wd_loss:
                self.layer_opt.set_wds(self.wds)
            else:
