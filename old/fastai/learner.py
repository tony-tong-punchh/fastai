...
class Learner:
    def __init__(
        ...
        self.tmp_path = (
            tmp_name if os.path.isabs(tmp_name) else os.path.join(self.data.path, tmp_name)
        )
        self.models_path = (
            models_name if os.path.isabs(models_name) else os.path.join(self.data.path, models_name)
        )
    ...
    def summary(self):
        return model_summary(self.model, [torch.rand(3, 3, self.data.sz, self.data.sz)])
    ...
    def fit_gen(
        ...
        **kwargs,
        ...
        if cycle_save_name:
            assert (
                use_clr or use_clr_beta or cycle_len
        ...
        if use_wd_sched:
            ...
            self.wd_sched = WeightDecaySchedule(
                layer_opt, batch_per_epoch, cl, cycle_mult, n_cycle, norm_wds, wds_sched_mult
            )
    ...
    def lr_find2(
        self, start_lr=1e-5, end_lr=10, num_it=100, wds=None, linear=False, stop_dv=True, **kwargs
    ):
        ...
        self.sched = LR_Finder2(
            layer_opt, num_it, end_lr, linear=linear, metrics=self.metrics, stop_dv=stop_dv
        )
        self.fit_gen(
            self.model,
            self.data,
