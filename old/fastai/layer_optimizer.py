...
class LayerOptimizer:
    ...
    def opt_params(self):
        assert len(self.layer_groups) == len(
            self.lrs
        ), f"size mismatch, expected {len(self.layer_groups)} lrs, but got {len(self.lrs)}"
        assert len(self.layer_groups) == len(
            self.wds
        ), f"size mismatch, expected {len(self.layer_groups)} wds, but got {len(self.wds)}"
...
def zip_strict_(l, r):
    assert len(l) == len(
        r
    ), f"size mismatch, expected lengths {len(l)}, but got {len(l)} and {len(r)} instead."
    return zip(l, r)
