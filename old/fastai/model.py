...
def set_train_mode(m):
    if hasattr(m, "running_mean") and (
        getattr(m, "bn_freeze", False) or not getattr(m, "trainable", False)
    ):
        m.eval()
    elif (
        getattr(m, "drop_freeze", False)
        and hasattr(m, "p")
    ...
    **kwargs,
    ...
    for epoch in tnrange(tot_epochs, desc="Epoch"):
        if phase >= len(n_epochs):
            break  # Sometimes cumulated errors make this append.
        ...
        if not all_val:
            ...
            if swa_model is not None:
                if (epoch + 1) >= swa_start and (
                    ...
                    swa_vals = validate(
                        swa_stepper, cur_data.val_dl, metrics, epoch, validate_skip=validate_skip
                    )
                    vals += swa_vals
...
def validate(stepper, dl, metrics, epoch, seq_first=False, validate_skip=0):
    if epoch < validate_skip:
        return [float("nan")] + [float("nan")] * len(metrics)
    batch_cnts, loss, res = [], [], []
    stepper.reset(False)
    with no_grad_context():
    ...
    return [np.average(loss, 0, weights=batch_cnts)[0]] + list(
        np.average(np.stack(res), 0, weights=batch_cnts)
    )
