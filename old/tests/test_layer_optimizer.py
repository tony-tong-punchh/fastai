...
def test_construction_with_lists_of_lrs_and_wds():
    lo = LayerOptimizer(FakeOpt, params_("A", "B", "C"), (1e-2, 2e-2, 3e-2), (9e-3, 8e-3, 7e-3),)
    check_optimizer_(
        lo.opt, [("A", 1e-2, 9e-3), ("B", 2e-2, 8e-3), ("C", 3e-2, 7e-3)],
    )
...
def test_set_lrs_with_list_of_values():
    ...
    check_optimizer_(
        lo.opt, [("A", 2e-2, 1e-4), ("B", 3e-2, 1e-4), ("C", 4e-2, 1e-4)],
    )
...
def test_set_wds_with_list_of_values():
    ...
    check_optimizer_(
        lo.opt, [("A", 1e-2, 9e-3), ("B", 1e-2, 8e-3), ("C", 1e-2, 7e-3)],
    )
