...
def csv_file(root_folder):
    tmp_csv_file = root_folder.mkdir("tmp_csv_folder").join("tmp_csv_file.csv")
    df_list = [
        {"id": 11 - i, "label": chr(ord("a") + 10 - i)} for i in range(1, 11)
    ]  # Create CSV with rows as (10, 'j'), (9, 'i') .. (1, 'a')
...
def test_image_classifier_data_from_csv_unsorted(root_folder, csv_file, data_folder):
    ...
    assert [os.path.split(o)[-1] for o in data.val_ds.fnames.tolist()] == val_fnames
