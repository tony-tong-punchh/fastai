...
def parse_args(args):
    arg_strs = [
        f'{arg.arg},{arg.annotation.id if arg.annotation else ""}'
    ...
    return "\n\n".join(
        [
            "{{" + f'arg {arg},{default if default else ""}' + "}}"
            for arg, default in zip(arg_strs, defaults)
        ]
    )
...
def gen_ascii_docs(src="fastai"):
    ...
    for file_path in file_paths:
        if pat.match(file_path.name):
            ...
            (file_path.parent / (file_path.name.rsplit(".", 1)[0] + ".adoc.tmpl")).write_text(
                tmpl_str
            )
            (file_path.parent / (file_path.name.rsplit(".", 1)[0] + ".adoc")).write_text(
                re.sub(r"{{(.*?)}}", parse_tmpl, tmpl_str, flags=re.DOTALL)
            )
    if path.is_dir():
...
_fn_lu = {"class": get_cls_str, "arg": get_sub_arg, "xref": get_xref_str, "method": get_method_str}
